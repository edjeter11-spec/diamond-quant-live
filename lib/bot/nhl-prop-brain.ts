...
export async function loadNHLPropBrainFromCloud(): Promise<NHLPropBrainState> {
  try {
    ...
    const data = await cloudGet<NHLPropBrainState | null>(
      "nhl_prop_brain",
      null,
    );
...
export async function saveNHLPropBrainToCloud(
  brain: NHLPropBrainState,
): Promise<void> {
  try {
    const { cloudSet } = await import("@/lib/supabase/client");
    await cloudSet("nhl_prop_brain", {
      ...brain,
...
function trimPlayerMemory(
  mem: Record<string, NHLPlayerMemory>,
): Record<string, NHLPlayerMemory> {
