...
export function projectProp(
  ...
  if (statKey === "pra") {
    avg =
      (playerStats.ppg ?? 0) + (playerStats.rpg ?? 0) + (playerStats.apg ?? 0);
  } else {
  ...
  const stdDev =
    recentForm?.variance && recentForm.variance > 0
      ? recentForm.variance
      : Math.max(avg * 0.3, 1.0);
  ...
  factors.push({
    name: "seasonAverage",
    signal: avgSignal,
    contribution: avgSignal * weights.seasonAverage,
  });
  ...
  if (recentForm && recentForm.gamesPlayed >= 5) {
    ...
    formSignal = Math.max(
      -1,
      Math.min(1, (recentForm.last5Avg - recentForm.seasonAvg) / stdDev),
    );
    // Bonus: if last 5 is trending way above season avg, strong over signal
    if (recentForm.last5Avg > recentForm.seasonAvg * 1.15)
      formSignal = Math.min(1, formSignal + 0.2);
    if (recentForm.last5Avg < recentForm.seasonAvg * 0.85)
      formSignal = Math.max(-1, formSignal - 0.2);
  } else {
  ...
  factors.push({
    name: "recentForm",
    signal: formSignal,
    contribution: formSignal * weights.recentForm,
  });
  ...
  factors.push({
    name: "matchupDefense",
    signal: defSignal,
    contribution: defSignal * weights.matchupDefense,
  });

  // ── Factor 4: Home/Away ──
  // Home players average ~1.5 more PPG, ~0.3 more RPG, ~0.2 more APG
  const homeBoost: Record<string, number> = {
    ppg: 1.5,
    rpg: 0.3,
    apg: 0.2,
    tpm: 0.2,
  ...
  factors.push({
    name: "homeAway",
    signal: Math.max(-1, Math.min(1, homeSignal)),
    contribution: Math.max(-1, Math.min(1, homeSignal)) * weights.homeAway,
  });
  ...
  factors.push({
    ...
    contribution: Math.max(-1, Math.min(1, restSignal)) * weights.restSchedule,
  });
  ...
  if (context.projectedGameTotal && context.leagueAvgTotal) {
    ...
    const paceBoost =
      (context.projectedGameTotal - context.leagueAvgTotal) /
      context.leagueAvgTotal;
  ...
  factors.push({
    ...
    contribution: Math.max(-1, Math.min(1, paceSignal)) * weights.paceContext,
  });

  // ── Factor 7: Line Movement ──
  let lineSignal = 0;
  if (
    context.lineOpenVsCurrent !== undefined &&
    context.lineOpenVsCurrent !== 0
  ) {
  ...
  factors.push({
    name: "lineMovement",
    signal: lineSignal,
    contribution: lineSignal * weights.lineMovement,
  });
  ...
  if (
    context.refCrew &&
    context.refCrew.length > 0 &&
    (statKey === "ppg" || statKey === "pra")
  ) {
    ...
    if (Math.abs(ftaDelta) >= 1) {
      ...
      reasoning.push(
        `Refs ${crew.names.slice(0, 3).join(", ")}: neutral FT impact`,
      );
    }
  }
  ...
  const confidence = Math.min(
    95,
    Math.round(Math.abs(probability - 0.5) * 200),
  );
