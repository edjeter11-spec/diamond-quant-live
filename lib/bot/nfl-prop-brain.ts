...
const DEFAULT_WEIGHTS: NFLPropWeights = {
  ...
  oppDefVsPosition: 0.2, // matchup matters huge in NFL
  weather: 0.1,
  restDays: 0.06,
  homeAway: 0.08,
  injuryRisk: 0.08,
...
export async function loadNFLPropBrainFromCloud(): Promise<NFLPropBrainState> {
  try {
    ...
    const data = await cloudGet<NFLPropBrainState | null>(
      "nfl_prop_brain",
      null,
    );
...
export async function saveNFLPropBrainToCloud(
  brain: NFLPropBrainState,
): Promise<void> {
  try {
    const { cloudSet } = await import("@/lib/supabase/client");
    await cloudSet("nfl_prop_brain", {
      ...brain,
...
function trimPlayerMemory(
  mem: Record<string, NFLPlayerMemory>,
): Record<string, NFLPlayerMemory> {
...
export function learnFromNFLResult(
  ...
  for (const f of result.factors) {
    ...
    updated.weights[k] = Math.max(
      ...
      Math.min(
        WEIGHT_CEILING,
        updated.weights[k] - updated.learningRate * error * f.signal,
      ),
    );
  }
