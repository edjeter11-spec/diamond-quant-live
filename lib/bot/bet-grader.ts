...
export function gradeLeg(
  ...
  if (m === "total" || m === "totals" || m === "over/under")
    return gradeTotal(pickText, game);
  if (
    m === "spread" ||
    m === "run line" ||
    m === "runline" ||
...
export function gradeBet(
  ...
  if (bet.isParlay || (bet.parlayLegs && bet.parlayLegs.length > 1)) {
    ...
    if (legs.length === 0)
      return { result: "pending", payout: 0, reason: "No legs listed" };
    ...
    if (legResults.some((r) => r === "pending")) {
      return {
        result: "pending",
        payout: 0,
        reason: "Some legs not yet settled",
      };
    }
  ...
  if (!game)
    return { result: "pending", payout: 0, reason: "Game not yet completed" };
  ...
  if (result === "pending") {
    return {
      result: "pending",
      payout: 0,
      reason: `Can't grade ${bet.market} from final score alone`,
    };
  }
