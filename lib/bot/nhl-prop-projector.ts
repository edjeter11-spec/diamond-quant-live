...
import {
  getNHLDefVsPosition,
  getNHLDefVsShots,
  getNHLPowerPlayGiven,
} from "@/lib/nhl/position-defense";
...
function getBaselineForMarket(p: NHLStarPlayer, market: string): number {
  switch (market) {
    ...
    case "player_goals":
      return p.goalsPerGame ?? 0;
    case "player_assists":
      return p.assistsPerGame ?? 0;
    case "player_shots_on_goal":
      return p.shotsPerGame ?? 0;
    case "player_total_saves":
      return p.savesPerGame ?? 0;
  }
  return 0;
}
...
export function projectNHLProp(
  ...
  // Signals live in [-1, +1]; contributions must use the SAME clamped value
  // so bad inputs (e.g. missing goalie SV%) can't exceed the factor weight.
  ...
  const oppDef =
    pos === "G"
      ? getNHLDefVsShots(context.oppAbbrev)
      : getNHLDefVsPosition(context.oppAbbrev, pos);
  ...
  if (
    ...
    // Guard: missing/absurd SV% (0, undefined) → neutral 0.910, otherwise a
    // missing feed reads as a +45 signal and swamps every other factor.
    const sv =
      Number.isFinite(context.oppGoalieSavePct) &&
      context.oppGoalieSavePct > 0.7 &&
      context.oppGoalieSavePct < 1
        ? context.oppGoalieSavePct
        : 0.91;
    const goalieEdge = clamp1((0.91 - sv) * 50); // ±0.02 SV% → ±1
    factors.push({
      name: "goalieMatchup",
      signal: goalieEdge,
      contribution: goalieEdge * weights.goalieMatchup,
      direction:
        goalieEdge < -0.05 ? "under" : goalieEdge > 0.05 ? "over" : "neutral",
      explanation: `Opp goalie SV% ${(sv * 100).toFixed(1)}% (avg 91.0)`,
    });
  } else if (pos === "G" && market === "player_total_saves") {
    // Facing high-shot team = more saves
    const shotsAllowSignal = clamp1((oppDef - 50) / 20);
    factors.push({
      name: "goalieMatchup",
  ...
  if (context.fatigue) {
    const fatigueSignal = clamp1(context.fatigue.fatigueEdge / 0.08);
    if (Math.abs(fatigueSignal) > 0.05) {
      factors.push({
        name: "fatigue",
        signal: fatigueSignal,
        contribution: fatigueSignal * weights.fatigue,
        direction: context.fatigue.isBackToBack
          ? "under"
          : context.fatigue.daysOfRest >= 3
            ? "over"
            : "neutral",
  ...
  // 7. Injury impact
  const injurySignal = clamp1((context.injuryFactor - 1) * 2);
  ...
  if (pos !== "G") {
    ...
    const ppSignal = clamp1(ppEdge / 2);
    factors.push({
      name: "powerPlayContext",
      signal: ppSignal,
      contribution: ppSignal * weights.powerPlayContext,
  ...
  const projectedValue = Math.max(0, seasonAvg * (1 + totalContribution * 0.3));

  // Probability via gaussian-ish
  const stdDev = Math.max(seasonAvg * 0.4, 0.3);
  ...
  const confidence = Math.min(
    100,
    Math.max(10, Math.abs(totalContribution) * 220),
  );
