...
const DIVISIONS: Record<string, string> = {
  "New York Yankees": "ALE",
  "Boston Red Sox": "ALE",
  "Toronto Blue Jays": "ALE",
  "Tampa Bay Rays": "ALE",
  "Baltimore Orioles": "ALE",
  "Cleveland Guardians": "ALC",
  "Minnesota Twins": "ALC",
  "Chicago White Sox": "ALC",
  "Detroit Tigers": "ALC",
  "Kansas City Royals": "ALC",
  "Houston Astros": "ALW",
  "Seattle Mariners": "ALW",
  "Texas Rangers": "ALW",
  "Los Angeles Angels": "ALW",
  Athletics: "ALW",
  "Oakland Athletics": "ALW",
  "Atlanta Braves": "NLE",
  "Philadelphia Phillies": "NLE",
  "New York Mets": "NLE",
  "Miami Marlins": "NLE",
  ...
  "Chicago Cubs": "NLC",
  "Cincinnati Reds": "NLC",
  "St. Louis Cardinals": "NLC",
  "Pittsburgh Pirates": "NLC",
  "Los Angeles Dodgers": "NLW",
  "San Diego Padres": "NLW",
  "San Francisco Giants": "NLW",
  "Arizona Diamondbacks": "NLW",
  "Colorado Rockies": "NLW",
};
...
async function fetchDetailedGames(
  startDate: string,
  endDate: string,
): Promise<DeepGameData[]> {
  ...
  for (const dateEntry of data.dates ?? []) {
    for (const game of dateEntry.games ?? []) {
      ...
      const isInterleague =
        homeDiv !== "" && awayDiv !== "" && homeDiv[0] !== awayDiv[0];
      ...
      games.push({
        ...
        homePitcher: hp
          ? {
              name: hp.fullName ?? "",
              era: parseFloat(hp.stats?.[0]?.splits?.[0]?.stat?.era ?? "4.5"),
              whip: parseFloat(hp.stats?.[0]?.splits?.[0]?.stat?.whip ?? "1.3"),
              hand: hp.pitchHand?.code ?? "R",
            }
          : null,
        awayPitcher: ap
          ? {
              name: ap.fullName ?? "",
              era: parseFloat(ap.stats?.[0]?.splits?.[0]?.stat?.era ?? "4.5"),
              whip: parseFloat(ap.stats?.[0]?.splits?.[0]?.stat?.whip ?? "1.3"),
              hand: ap.pitchHand?.code ?? "R",
            }
          : null,
...
export async function deepTrain(
  ...
  for (const season of seasons) {
    ...
    for (const month of months) {
      ...
      onProgress?.(
        `Deep analyzing ${season} ${["", "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"][month]}...`,
      );
      ...
      for (const g of games) {
        ...
        if (g.homePitcher && g.awayPitcher) {
          ...
          state = learnFromResult(state, {
            market: "moneyline",
            predictedProb: Math.min(
              0.75,
              Math.max(0.25, 0.52 + eraDiff * 0.04),
            ),
            won: betterHome === g.homeWon,
            ev:
              Math.abs(eraDiff) > 1
                ? 4 * learningMultiplier
                : 2 * learningMultiplier,
          });
          ...
          state = learnFromResult(state, {
            ...
            predictedProb: Math.min(
              0.65,
              Math.max(0.35, 0.5 + whipDiff * 0.08),
            ),
          ...
          if (g.homePitcher.era < 3.0 || g.awayPitcher.era < 3.0) {
            c.lowERATotal++;
            if (
              (g.homePitcher.era < 3.0 && g.homeWon) ||
              (g.awayPitcher.era < 3.0 && !g.homeWon)
            )
              c.lowERAWin++;
          }
          // High ERA losses
          if (g.homePitcher.era > 5.0 || g.awayPitcher.era > 5.0) {
            c.highERATotal++;
            if (
              (g.homePitcher.era > 5.0 && !g.homeWon) ||
              (g.awayPitcher.era > 5.0 && g.homeWon)
            )
              c.highERALoss++;
          }
          // Ace vs Ace
          if (g.homePitcher.era < 3.5 && g.awayPitcher.era < 3.5) {
            c.aceTotal++;
            if (g.homeWon) c.aceHome++;
          }
          // Bullpen game
          if (g.homePitcher.era > 4.5 && g.awayPitcher.era > 4.5) {
            c.bullpenTotal++;
            if (g.homeWon) c.bullpenHome++;
          }

          // ═══ FACTOR 10: Lefty/Righty ═══
          if (g.homePitcher.hand === "L") {
            c.lhpTotal++;
            if (g.homeWon) c.lhpHome++;
          } else {
            c.rhpTotal++;
            if (g.homeWon) c.rhpHome++;
          }
        }
        ...
        state = learnFromResult(state, {
          market: "total",
          predictedProb:
            g.parkFactor > 1.05 ? 0.56 : g.parkFactor < 0.95 ? 0.44 : 0.5,
        ...
        if (g.dayNight === "day") {
          c.dayTotal++;
          if (g.homeWon) c.dayHome++;
        } else {
          c.nightTotal++;
          if (g.homeWon) c.nightHome++;
        }
        state = learnFromResult(state, {
          market: "moneyline",
        ...
        if (g.dayOfWeek === 0) {
          c.sunTotal++;
          if (g.homeWon) c.sunHome++;
        }
        if (g.dayOfWeek === 5) {
          c.friTotal++;
          if (g.homeWon) c.friHome++;
        }
        ...
        if (g.isDivisional) {
          c.divTotal++;
          if (g.homeWon) c.divHome++;
        }
        if (g.isInterleague) {
          c.interTotal++;
          if (g.homeWon) c.interHome++;
        }
        state = learnFromResult(state, {
          market: "moneyline",
        ...
        if (g.runDiff === 1) {
          c.oneRunTotal++;
          if (g.homeWon) c.oneRunHome++;
          state = learnFromResult(state, {
            market: "moneyline",
            predictedProb: 0.54,
            won: g.homeWon,
            ev: 3 * learningMultiplier,
          });
        }
        ...
        if (homeLeadingAfter5) {
          c.holdOpps++;
          if (g.homeWon) c.holds++;
        }
        if (awayLeadingAfter5) {
          c.comebackOpps++;
          if (g.homeWon) c.comebacks++;
        }
        ...
        if (g.month <= 4) {
          c.aprTotal++;
          if (g.homeWon) c.aprHome++;
        }

        // ═══ FACTOR 13: Doubleheader game 2 ═══
        if (g.gameNumber === 2) {
          c.dh2Total++;
          if (g.homeWon) c.dh2Home++;
          state = learnFromResult(state, {
            market: "moneyline",
            predictedProb: 0.5,
            won: g.homeWon,
            ev: 2 * learningMultiplier,
          });
        }
        ...
        for (const pitcher of [g.homePitcher, g.awayPitcher]) {
          ...
          pm.gamesTracked++;
          const pWon =
            (pitcher === g.homePitcher && g.homeWon) ||
            (pitcher === g.awayPitcher && !g.homeWon);
          if (pWon) pm.wins++;
          else pm.losses++;
          pm.winRate =
            pm.gamesTracked > 0
        ...
        if (g.venue) {
          ...
          pm.nrfiRate =
            pm.nrfiRate * (1 - a) + (firstInningClean ? 100 : 0) * a;
        }
  ...
  if (state.pitcherMemory) {
    ...
    if (keys.length > 300) {
      const sorted = keys.sort(
        (a, b) =>
          (state.pitcherMemory[b]?.gamesTracked ?? 0) -
          (state.pitcherMemory[a]?.gamesTracked ?? 0),
      );
      const keep = new Set(sorted.slice(0, 250));
      for (const k of keys) {
        if (!keep.has(k)) delete state.pitcherMemory[k];
      }
    }
  }
  ...
  const patterns: LearnedPatterns = {
    ...
    avgRunsPerGame:
      c.total > 0 ? Math.round((c.runsSum / c.total) * 100) / 100 : 8.5,
