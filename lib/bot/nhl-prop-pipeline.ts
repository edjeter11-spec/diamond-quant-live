...
export async function commitNHLPropProjections(
  ...
  for (const g of NHL_STAR_FALLBACK.filter((p) => p.position === "G")) {
    if (!teamGoaliePct.has(g.team))
      teamGoaliePct.set(g.team, g.savePct ?? 0.91);
  }
  ...
  for (const star of stars) {
    ...
    const keyOuts = ownInjuries.filter((p) =>
    ...
    const oppGoalieSavePct = teamGoaliePct.get(oppAbbrev) ?? 0.91;
    ...
    for (const market of NHL_MARKETS) {
      ...
      switch (market) {
        case "player_points":
          baseline = star.pointsPerGame ?? 0;
          break;
        case "player_goals":
          baseline = star.goalsPerGame ?? 0;
          break;
        case "player_assists":
          baseline = star.assistsPerGame ?? 0;
          break;
        case "player_shots_on_goal":
          baseline = star.shotsPerGame ?? 0;
          break;
        case "player_total_saves":
          baseline = star.savesPerGame ?? 0;
          break;
      }
      ...
      const proj = projectNHLProp(star, market, line, brain.weights, {
        oppAbbrev,
        isHome,
        fatigue,
        injuryFactor,
      ...
      if (proj.confidence < 22) {
        skipped++;
        continue;
      }
      if (Math.abs(proj.probability - 0.5) < 0.07) {
        skipped++;
        continue;
      }
