...
export async function commitNFLPropProjections(
  ...
  for (const t of injuryReports)
    for (const p of t.players) {
      if (
        p.status.toLowerCase().includes("out") ||
        p.status.toLowerCase().includes("ir")
      ) {
  ...
  for (const star of stars) {
    if (outNames.has(star.playerName.toLowerCase())) {
      skipped++;
      continue;
    }
    ...
    const game = games.find(
      (g) => g.homeAbbrev === star.team || g.awayAbbrev === star.team,
    );
    if (!game) continue;
    ...
    const keyOuts = ownInjuries.filter(
      (p) =>
        p.status.toLowerCase().includes("out") &&
        ["QB", "WR", "RB", "TE", "LT", "LG", "C", "RG", "RT"].includes(
          p.position,
        ),
    ).length;
    ...
    for (const market of NFL_MARKETS) {
      ...
      switch (market) {
        case "player_pass_yds":
          baseline = star.passYds ?? 0;
          break;
        case "player_pass_tds":
          baseline = star.passTds ?? 0;
          break;
        case "player_pass_attempts":
          baseline = star.passAttempts ?? 0;
          break;
        case "player_rush_yds":
          baseline = star.rushYds ?? 0;
          break;
        case "player_rush_attempts":
          baseline = star.rushAttempts ?? 0;
          break;
        case "player_receptions":
          baseline = star.receptions ?? 0;
          break;
        case "player_reception_yds":
          baseline = star.receivingYds ?? 0;
          break;
      }
      ...
      const proj = projectNFLProp(star, market, line, brain.weights, {
        oppAbbrev,
        isHome,
        weather,
        rest,
      ...
      if (proj.confidence < 25) {
        skipped++;
        continue;
      }
      if (Math.abs(proj.probability - 0.5) < 0.08) {
        skipped++;
        continue;
      }
      ...
      const evEdge =
        ((proj.probability - impliedProb) / Math.max(impliedProb, 0.01)) * 100;
...
const MARKET_TO_STAT: Record<
  string,
  keyof Awaited<ReturnType<typeof fetchNFLBoxScore>>[0]["stats"]
> = {
  ...
  for (const [gameId, preds] of byGame.entries()) {
    try {
      ...
      for (const pred of preds) {
        ...
        const player = box.find(
          (b) =>
            b.playerName.toLowerCase() === lower ||
            b.playerName.toLowerCase().includes(lastName),
        );
        if (!player) continue;
