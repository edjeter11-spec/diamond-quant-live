...
const REFEREE_TENDENCIES: Record<string, Omit<RefereeData, "name">> = {
  ...
  "Scott Foster": {
    foulRatePerGame: 46,
    totalPointsBoost: 229,
    ftAttemptsBoost: 50,
  },
  "Tony Brothers": {
    foulRatePerGame: 44,
    totalPointsBoost: 227,
    ftAttemptsBoost: 48,
  },
  "Marc Davis": {
    foulRatePerGame: 45,
    totalPointsBoost: 228,
    ftAttemptsBoost: 49,
  },
  "Eric Lewis": {
    foulRatePerGame: 44,
...
const refereeCache = new Map<
  string,
  { data: RefereeData[]; cachedAt: number }
>();
const CACHE_TTL_MS = 30 * 60 * 1000; // 30 min

export async function getGameReferees(
  ...
  try {
    ...
    const officials: any[] =
      data?.gameInfo?.officials ?? data?.boxscore?.officials ?? [];
...
export function aggregateCrew(
  refs: RefereeData[],
): Omit<RefereeData, "name"> & { names: string[] } {
...
export function describeCrew(refs: RefereeData[]): string {
  ...
  const tag =
    dev > 1.5
      ? "high-foul crew (over lean)"
      : dev < -1.5
        ? "let-them-play crew (under lean)"
        : "neutral crew";
