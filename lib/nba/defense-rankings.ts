...
let cache: { rankings: Record<string, TeamDefenseRanking>; ts: number } | null =
  null;

export async function fetchDefenseRankings(): Promise<
  Record<string, TeamDefenseRanking>
> {
    ...
    const results = await Promise.all(
      batch.map(async ([abbrev, id]) => {
        try {
          ...
          const offStats =
            stats.find((c: any) => c.name === "offensive")?.stats ?? [];
          const genStats =
            stats.find((c: any) => c.name === "general")?.stats ?? [];

          const avgPts =
            offStats.find((s: any) => s.name === "avgPoints")?.value ?? 110;
          const avgReb =
            genStats.find((s: any) => s.name === "avgRebounds")?.value ?? 44;
          const avgAst =
            offStats.find((s: any) => s.name === "avgAssists")?.value ?? 25;
  ...
  try {
    ...
    await cloudSet("nba_defense_rankings", {
      rankings: result,
...
export async function getDefenseRank(
  teamAbbrev: string,
  propType: string,
): Promise<number> {
