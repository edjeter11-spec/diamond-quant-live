...
const umpireCache = new Map<
  string,
  { data: UmpireData | null; cachedAt: number }
>();
const CACHE_TTL_MS = 30 * 60 * 1000; // 30 min

export async function getHomePlateUmpire(
  gamePk: string | number,
): Promise<UmpireData | null> {
...
export function describeUmpire(ump: UmpireData): string {
  ...
  if (dev > 0.4)
    return `${ump.name} — hitter-friendly zone (${ump.runScoringIndex.toFixed(1)} R/G avg, ${ump.kZoneAccuracy}% accuracy)`;
  if (dev < -0.4)
    return `${ump.name} — tight zone, pitcher-friendly (${ump.runScoringIndex.toFixed(1)} R/G avg, ${ump.kZoneAccuracy}% accuracy)`;
