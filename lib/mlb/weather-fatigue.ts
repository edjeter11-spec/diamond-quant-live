...
const STADIUMS: Record<
  string,
  { lat: number; lon: number; name: string; roof: boolean }
> = {
  ...
  DET: { lat: 42.339, lon: -83.0485, name: "Comerica Park", roof: false },
  ...
  LAD: { lat: 34.0739, lon: -118.24, name: "Dodger Stadium", roof: false },
  MIA: { lat: 25.7781, lon: -80.2197, name: "LoanDepot Park", roof: true },
  MIL: { lat: 43.028, lon: -87.9712, name: "American Family", roof: true },
  ...
  SD: { lat: 32.7076, lon: -117.157, name: "Petco Park", roof: false },
...
export async function getGameWeather(
  ...
  try {
    ...
    try {
      const { applyParkWeatherInteraction } =
        await import("@/lib/mlb/park-factors");
...
function buildWeatherSummary(
  ...
  if (stadium.roof)
    return `${stadium.name} has a retractable roof — weather is neutralized.`;
  ...
  if (windSpeed > 10) {
    ...
    if (["S", "SW", "SE"].includes(windDir))
      parts.push("— blowing out, expect more fly balls to carry");
    else if (["N", "NW", "NE"].includes(windDir))
      parts.push("— blowing in, holds fly balls in the park");
  }

  if (temp > 85)
    parts.push("Hot day — ball carries further, expect more offense");
  else if (temp < 55) parts.push("Cold and heavy air — favors pitchers");

  if (condition === "Rain")
    parts.push("Rain expected — slick balls, possible delays");

  if (hitImpact > pitchImpact)
    parts.push(`Net: +${hitImpact - pitchImpact} hitter-friendly`);
  else if (pitchImpact > hitImpact)
    parts.push(`Net: +${pitchImpact - hitImpact} pitcher-friendly`);
...
export async function getTeamFatigue(
  teamName: string,
  ...
  try {
    ...
    const summary = buildFatigueSummary(
      teamName,
      last3Days,
      last7Days,
      isB2B,
...
function buildFatigueSummary(
  ...
  if (b2b2b)
    parts.push(
      `${team} on a grueling stretch — ${last3} games in 3 days, ${last7} in the last week`,
    );
  else if (b2b)
    parts.push(`${team} playing back-to-back — ${last3} games in 3 days`);
  ...
  if (fatigue > 50)
    parts.push(
      "HIGH fatigue risk — bullpen likely taxed, expect more runs late",
    );
