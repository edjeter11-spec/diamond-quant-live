...
function ProfitChart({
  ...
  const path = points
    .map(
      (p, i) =>
        `${i === 0 ? "M" : "L"}${(i * stepX).toFixed(1)},${y(p).toFixed(1)}`,
    )
    .join(" ");
    ...
    <div className="space-y-2">
      <div className="flex items-center gap-2 text-xs">
        <span className="text-mercury/60">
          If you'd bet $100 on every pick at -110:
        </span>
        <span
          className={`font-mono font-bold ml-auto ${final >= 0 ? "text-neon" : "text-danger"}`}
        >
      ...
      >
        ...
        <line
          ...
          x2={W}
          y2={zeroY}
          stroke="rgba(196,200,216,0.2)"
          strokeDasharray="2,3"
        />
        {/* Profit line */}
        <path
          d={path}
          stroke={final >= 0 ? "#2ee6a6" : "#ff5c7a"}
          strokeWidth="2"
          fill="none"
        />
        {/* Area under */}
        <path
          ...
          fill={final >= 0 ? "rgba(0,255,136,0.08)" : "rgba(255,59,92,0.08)"}
        />
      </svg>
...
export default function BrainPage() {
  ...
  const [propHistory, setPropHistory] = useState<
    Array<{ result?: string; date: string }>
  >([]);
  ...
  if (!stats?.ok || !stats.brain) {
    return (
      <div className="min-h-screen bg-bunker flex flex-col items-center justify-center gap-3">
        ...
        <Link href="/" className="text-electric text-xs">
          ← back to dashboard
        </Link>
      </div>
    );
  ...
  const overallAccuracy =
    brain.totalPredictions > 0
      ? (brain.totalHits / brain.totalPredictions) * 100
      : 0;
  const weightEntries = Object.entries(brain.weights).sort(
    (a, b) => b[1] - a[1],
  );
    ...
    <div className="min-h-screen bg-bunker pb-20">
      ...
      <div className="sticky top-0 z-10 bg-bunker/95 backdrop-blur border-b border-slate/30 px-4 py-3">
        <div className="max-w-5xl mx-auto flex items-center gap-3">
          <Link
            href="/"
            className="min-w-[44px] min-h-[44px] flex items-center justify-center -ml-2 text-mercury hover:text-silver"
          ...
          <div className="flex-1">
            ...
            <p className="text-[10px] text-mercury/60">
              v{brain.version} • {brain.totalGamesProcessed.toLocaleString()}{" "}
              games trained
            </p>
          </div>
          <div className="text-right">
            <p
              className={`text-lg font-bold font-mono ${overallAccuracy > 52 ? "text-neon" : overallAccuracy > 48 ? "text-electric" : "text-amber"}`}
            >
              {overallAccuracy.toFixed(1)}%
            </p>
            <p className="text-[9px] text-mercury/50">
              {brain.totalPredictions.toLocaleString()} predictions
            </p>
          </div>
        </div>
      </div>
        ...
        >
          ...
          <div className="glass rounded-xl p-4 border border-purple/15">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 className="text-xs font-bold text-silver uppercase tracking-wider">
            ...
            <div className="grid grid-cols-3 gap-3">
              {Object.entries(brain.markets ?? {}).map(([key, m]) => (
                <div
                  key={key}
                  className="text-center p-3 rounded-lg bg-gunmetal/30 border border-slate/15"
                >
                  <p
                    className={`text-2xl font-bold font-mono ${m.winRate > 52 ? "text-neon" : m.winRate > 48 ? "text-electric" : "text-amber"}`}
                  >
                    {m.winRate.toFixed(1)}%
                  </p>
                  <p className="text-[10px] text-mercury/60 capitalize mt-1">
                    {key.replace("player_", "")}
                  </p>
            ...
            <div className="glass rounded-xl p-4 border border-neon/15">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-[9px] text-mercury/40 ml-auto">
                  {stats.brain.playerCount} tracked
                </span>
              </div>
              <div className="rounded-lg overflow-hidden border border-slate/15">
                {topPlayers.slice(0, 5).map((p, i) => (
                  <div
                    key={p.name}
                    className={`flex items-center gap-3 px-3 py-2.5 text-xs border-b border-slate/10 last:border-0 ${i < 3 ? "bg-neon/5" : ""}`}
                  >
                    <span className="w-5 text-mercury/40 font-mono text-[10px]">
                      #{i + 1}
                    </span>
                    <div className="flex-1 min-w-0">
                      <p className="text-silver font-semibold truncate">
                        {p.name}
                      </p>
                      <p className="text-[10px] text-mercury/50 truncate">
                        {p.team} • {p.total} picks
                      </p>
                    </div>
                    <span
                      className={`font-mono font-bold ${p.winRate > 55 ? "text-neon" : p.winRate > 50 ? "text-electric" : "text-amber"}`}
                    >
                      {p.winRate.toFixed(0)}%
                    </span>
                  </div>
