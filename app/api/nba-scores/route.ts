...
function mapStatus(name: string): "pre" | "live" | "final" {
  ...
  if (name === "STATUS_IN_PROGRESS" || name === "STATUS_HALFTIME")
    return "live";
  return "pre";
}
...
function mapEvent(event: any) {
  ...
  const homeAbbrev: string =
    home.team?.abbreviation ?? getNBATeamAbbrev(home.team?.displayName ?? "");
  const awayAbbrev: string =
    away.team?.abbreviation ?? getNBATeamAbbrev(away.team?.displayName ?? "");
  ...
  const detailedStatus =
    status === "final"
      ? "Final"
      : status === "live"
        ? `${periodLabel} ${timeRemaining}`.trim()
...
export async function GET() {
  try {
    ...
    let games = todayEvents
      .map(mapEvent)
      .filter((g) => isActive(g, now, STALE_MS));
