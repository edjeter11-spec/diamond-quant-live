...
import { buildPickMath } from "@/lib/bot/prop-reasoning";
  ...
  try {
    ...
    if (apiKey) {
      try {
        ...
        console.error(
          "NBA odds fetch failed, falling back to free events:",
    ...
    const futureGames = rawGames.filter((g) => {
      ...
      const gameDay = new Date(g.commence_time).toLocaleDateString("en-US", {
        timeZone: "America/New_York",
      });
    ...
    const analyses = await Promise.all(
      futureGames.slice(0, 12).map(async (game) => {
        ...
        const restEdge =
          homeRest && awayRest
            ? computeRestEdge(homeRest, awayRest)
            : { edge: 0, factors: [] };
        ...
        if (
          homeLineup &&
          (homeLineup.confirmed || homeLineup.inactiveStars.length > 0)
        ) {
          lineupEdge += computeNBALineupEdge(homeLineup);
          if (homeLineup.inactiveStars.length > 0)
            lineupFactors.push(homeLineup.summary);
        }
        if (
          awayLineup &&
          (awayLineup.confirmed || awayLineup.inactiveStars.length > 0)
        ) {
          // Inactives on AWAY help HOME — flip sign
          lineupEdge -= computeNBALineupEdge(awayLineup);
          if (awayLineup.inactiveStars.length > 0)
            lineupFactors.push(awayLineup.summary);
        }
        ...
        for (const inj of homeInjuries ?? []) {
          if (inj.status === "Out") {
            injuryEdge -= 1.5;
            injuryFactors.push(`${homeAbbrev} OUT: ${inj.name}`);
          } else if (inj.status === "Doubtful") {
            injuryEdge -= 1.0;
            injuryFactors.push(`${homeAbbrev} DOUBTFUL: ${inj.name}`);
        ...
        for (const inj of awayInjuries ?? []) {
          if (inj.status === "Out") {
            injuryEdge += 1.5;
            injuryFactors.push(`${awayAbbrev} OUT: ${inj.name}`);
          } else if (inj.status === "Doubtful") {
            injuryEdge += 1.0;
            injuryFactors.push(`${awayAbbrev} DOUBTFUL: ${inj.name}`);
          } else if (inj.status === "Questionable") {
            injuryEdge += 0.4;
          }
        }
        ...
        const marketModel = {
          ...
          factors: [
            `Market consensus from ${homeProbs.length} books: ${(marketProb * 100).toFixed(1)}% home`,
          ],
        };
        ...
        if (restEdge.edge !== 0) {
          ...
          formModel.homeWinProb = Math.min(
            0.95,
            Math.max(0.05, formModel.homeWinProb + probShift),
          );
        ...
        if (injuryEdge !== 0) {
          ...
          netRatingModel.homeWinProb = Math.min(
            0.95,
            Math.max(0.05, netRatingModel.homeWinProb + probShift),
          );
          netRatingModel.factors.push(...injuryFactors.slice(0, 4));
          netRatingModel.confidence = Math.min(
            80,
            netRatingModel.confidence + Math.abs(injuryEdge) * 3,
          );
        }
        ...
        if (lineupEdge !== 0) {
          ...
          netRatingModel.confidence = Math.min(
            85,
            netRatingModel.confidence + Math.abs(lineupEdge) * 2,
          );
          // Also tilt the projected total — fewer stars means fewer points
          const totalDrop = Math.abs(lineupEdge) * 0.5;
          netRatingModel.totalProjection = Math.max(
            180,
            netRatingModel.totalProjection - totalDrop,
          );
        }
        ...
        marketModel.totalProjection =
          (marketModel.totalProjection + paceProj.projectedTotal) / 2;
        ...
        if (Math.abs(netGap) >= 1) {
          ...
          netRatingModel.homeWinProb = Math.min(
            0.92,
            Math.max(0.08, netRatingModel.homeWinProb + ratingShift),
          );
          netRatingModel.factors.push(
            `Net rating: ${homeAbbrev} ${homeR.netRating > 0 ? "+" : ""}${homeR.netRating} vs ${awayAbbrev} ${awayR.netRating > 0 ? "+" : ""}${awayR.netRating}`,
          );
        }

        // Consensus
        const consensus = buildNBAConsensus(
          netRatingModel,
          marketModel,
          formModel,
        );
        ...
        const picks: any[] = [];
        if (
          consensus.confidence !== "NO_PLAY" &&
          bestHomeML.odds !== -Infinity
        ) {
          ...
          const oppOdds = isHome ? bestAwayML.odds : bestHomeML.odds;
          const mlMath = buildPickMath(
            isHome ? consensus.homeWinProb : 1 - consensus.homeWinProb,
            pickOdds,
            oppOdds !== -Infinity ? oppOdds : undefined,
          );
          ...
          picks.push({
            ...
            bestOdds: pickOdds,
            marketOdds: pickOdds,
            modelProb: mlMath.modelProb,
            impliedProb: mlMath.impliedProb,
            fairOdds: mlMath.fairOdds,
            edgePct: mlMath.edgePct,
            evPct: mlMath.evPct,
          });
        }
        ...
        if (oddsLines[0]?.homeSpread) {
          const spreadOdds = oddsLines[0]?.spreadPrice ?? -110;
          const spreadMath = buildPickMath(0.5, spreadOdds, spreadOdds);
          picks.push({
            ...
            // ── Math breakdown + sportsbook value (additive) ──
            bestBook: oddsLines[0]?.bookmaker ?? "",
            bestOdds: spreadOdds,
            marketOdds: spreadOdds,
            modelProb: spreadMath.modelProb,
            impliedProb: spreadMath.impliedProb,
            fairOdds: spreadMath.fairOdds,
            edgePct: spreadMath.edgePct,
            evPct: spreadMath.evPct,
          });
        }
    ...
    const response = {
      ...
      highConfidence: analyses.filter((a) => a.consensus.confidence === "HIGH")
        .length,
