...
import { fetchNFLInjuries } from "@/lib/nfl/injuries";
...
async function fetchESPNNFLNews(): Promise<NewsItem[]> {
  try {
...
export async function GET() {
  const cached = getCached("nfl_news_combined", 300_000);
  if (cached) return NextResponse.json(cached);

  try {
    const [reports, espn] = await Promise.all([
      fetchNFLInjuries().catch(() => []),
      fetchESPNNFLNews(),
    ]);
    ...
    for (const t of reports) {
      for (const p of t.players) {
        ...
        const priority = lower.includes("out") ? 10 : lower.includes("doubtful") ? 8 : lower.includes("questionable") ? 5 : 3;
        injuryItems.push({
          type: "injury",
          player: p.name,
          team: t.team,
    ...
    const response = {
      ...
      counts: {
        ...
        questionable: injuryItems.filter((i) => i.status?.toLowerCase().includes("questionable")).length,
        general: espn.length,
      },
      generatedAt: new Date().toISOString(),
    };

    setCache("nfl_news_combined", response);
