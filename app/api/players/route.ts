...
export async function GET(req: Request) {
  ...
  //
  // v6: bumped when MLB projections were added. Entries written by the
  // previous deploy carry no brainOverProb, and with a 60-minute TTL (plus a
  // day-long Supabase snapshot) they would have kept serving un-projected
  ...
  const cacheKey = `props_v8_${sport}_${market}`;
  ...
  const snapshotKey = `props_snap_v6_${sport}_${market}_${todayKey()}`;
  try {
