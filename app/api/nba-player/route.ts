...
export async function GET(req: Request) {
  ...
  const line =
    Number.isFinite(lineRaw) && lineRaw > 0 && lineRaw < 100 ? lineRaw : 0;
  ...
  try {
    ...
    const avg5 =
      last5.length > 0
        ? last5.reduce((s: number, g: any) => s + getter(g), 0) / last5.length
        : 0;
    const avgF5 =
      first5.length > 0
        ? first5.reduce((s: number, g: any) => s + getter(g), 0) / first5.length
        : 0;
    ...
    if (hitRate >= 70 && avg > line + 1) {
      side = "over";
      confidence = Math.min(hitRate, 85);
      ...
      side = "under";
      confidence = Math.min(100 - hitRate, 85);
    ...
    if (trend === "down")
      reasons.push("Trending down — recent performance declining");
    ...
    // ── Brain projection ──
    try {
      const { loadNbaPropBrainFromCloud } =
        await import("@/lib/bot/nba-prop-brain");
      ...
      if (brain?.weights && line > 0) {
        const statKey =
          market === "player_points"
            ...
            : market === "player_rebounds"
              ...
              : "apg";
        const seasonAvg =
          (profile as any)[statKey] ?? profile.statAvg[market] ?? line;
        ...
        const last5Avg =
          last5.length > 0
            ? last5.reduce((s: number, g: any) => s + getVal(g), 0) /
              last5.length
            : undefined;
          ...
          last5.length >= 5
            ? {
                ...
                last10Avg:
                  profile.gameLog
                    .slice(0, 10)
                    .reduce((s: number, g: any) => s + getVal(g), 0) /
                  Math.min(profile.gameLog.length, 10),
                seasonAvg,
