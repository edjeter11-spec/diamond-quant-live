...
export default function SlipPage({
    ...
    return (
      <div className="min-h-screen flex items-center justify-center text-center px-4">
        <div>
          ...
          <Link
            href="/"
            className="text-xs text-electric hover:text-neon mt-2 inline-block"
          >
            ← Back to Dashboard
          </Link>
        </div>
  ...
  const toPayout = (odds: number, bet: number) =>
    odds > 0 ? bet * (odds / 100) : bet * (100 / Math.abs(odds));
  const payout =
    stake && totalOdds
      ? Math.round(toPayout(totalOdds, stake) * 100) / 100
      : null;
    ...
    <div className="min-h-screen text-silver">
      <div className="max-w-lg mx-auto px-4 pt-6 pb-8">
        ...
        <div className="flex items-center gap-3 mb-6">
          <Link
            href="/"
            className="p-2 rounded-lg hover:bg-gunmetal/60 transition-colors"
          >
          ...
          <div className="flex-1">
            ...
            <p className="text-[10px] text-mercury">
              {slip.slip_data.sharedBy ?? "Anonymous"} •{" "}
            ...
            {picks.map((pick, i) => (
              <div key={i} className="px-4 py-3 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-mercury truncate">
                    {pick.game}
                  </p>
                </div>
        ...
        <div className="flex items-center justify-center gap-3 mb-6">
          {[
            ...
            },
            {
              ...
              icon: Skull,
              color: "text-mercury",
              bg: "bg-mercury/10 border-mercury/20",
            },
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-4 py-2 rounded-xl border transition-all ${
                reacted === emoji
                  ? `${bg} scale-110`
                  : `bg-bunker border-slate/40 hover:${bg}`
              } disabled:opacity-70`}
            >
              <Icon className={`w-4 h-4 ${color}`} />
              <span className="text-xs font-bold text-white">
                {slip.reactions[emoji] ?? 0}
              </span>
            </button>
          ))}
        </div>
        ...
        {payout !== null && stake && (
          <div className="rounded-xl bg-bunker border border-slate/40 p-4 mb-4 flex items-center justify-between">
            <div>
              <p className="text-[10px] text-mercury uppercase tracking-wider">
                Risk
              </p>
        ...
        <Link
          href="/"
          className="block rounded-xl bg-gradient-to-br from-neon/15 to-electric/10 border border-neon/25 p-5 hover:border-neon/50 transition-all group"
        >
          <div className="flex items-center gap-3 mb-2">
            ...
            <div className="flex-1">
              <p className="text-sm font-bold text-white">
                Get your own +EV picks
              </p>
              <p className="text-[10px] text-mercury">
                Live odds across 10+ books · Quant models · Free
              </p>
            </div>
        ...
        <div className="text-center mt-4">
          <Link
            href="/"
            className="text-[10px] text-mercury hover:text-silver transition-colors"
          >
