...
for (const date of [yesterday, today]) {
  console.log(`\n=== manual_picks for ${date} ===`);
  const rows = await j(
    `manual_picks?select=id,sport,slate_date,status,result,pick_text,batch_key&slate_date=eq.${date}&status=eq.published&order=sport,batch_key`,
  );
