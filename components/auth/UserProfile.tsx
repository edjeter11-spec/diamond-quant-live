      ...
      {loading ? (
        ...
        <div className="p-6 text-center">
          ...
          <p className="text-xs text-mercury">
            Start tracking picks to see your stats
          </p>
          <Link
            href="/?tab=bankroll"
        ...
        <div className="p-4 space-y-4">
          ...
          {stats.favoriteMarket && (
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-semibold text-silver">
                {stats.favoriteMarket}
              </span>
            </div>
          )}
          ...
          {(stats.mlbPct > 0 || stats.nbaPct > 0) && (
            <div className="space-y-1.5">
              ...
              <div className="h-1.5 rounded-full bg-gunmetal/50 overflow-hidden flex">
                ...
                />
                <div className="bg-purple transition-all flex-1" />
              </div>
            </div>
          )}
        </div>
...
export default function UserProfile() {
  ...
  const [pushMinConf, setPushMinConf] = useState(
    preferences?.push_min_confidence ?? "MEDIUM",
  );
  const [emailRecap, setEmailRecap] = useState(
    preferences?.email_daily_recap ?? false,
  );
  const [defaultSport, setDefaultSport] = useState(
    preferences?.default_sport ?? "mlb",
  );
  ...
  if (!user || !profile) {
    return (
      <div className="glass rounded-xl p-8 text-center">
        ...
        <p className="text-xs text-mercury/50 mt-1">
          Your picks, bankroll, and settings sync across devices
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      <div className="glass rounded-xl overflow-hidden border border-electric/20">
        <div className="px-4 py-3 bg-gradient-to-r from-electric/10 to-purple/5 border-b border-electric/15 flex items-center gap-3">
          {profile.avatar_url ? (
            <img
              ...
              alt=""
              className="w-10 h-10 rounded-full border border-neon/30"
            />
          ) : (
          ...
          <div className="flex-1">
            ...
            <p className="text-[9px] text-mercury/40">
        ...
        <div className="grid grid-cols-4 gap-px bg-slate/10">
          ...
          <StatBox
            ...
            color={winRate > 52 ? "text-neon" : "text-silver"}
          />
          <StatBox
            label="ROI"
            value={`${roi >= 0 ? "+" : ""}${roi.toFixed(1)}%`}
            color={roi > 0 ? "text-neon" : "text-danger"}
          />
          <StatBox
            label="Profit"
            value={`${profit >= 0 ? "+" : ""}$${profit.toFixed(0)}`}
            color={profit >= 0 ? "text-neon" : "text-danger"}
          />
        </div>
      </div>
      ...
      {(profile as any)?.stripe_customer_id && (
        <div className="glass rounded-xl p-4 space-y-3">
          ...
          <div className="flex items-center justify-between">
            ...
            <SubscriptionPauseButton
              paused={(profile as any).subscription_status === "paused"}
            />
          </div>
        </div>
      )}
      ...
      <div className="glass rounded-xl p-4 space-y-3">
        ...
        <div className="space-y-2">
          <label className="text-[10px] text-mercury/60 uppercase">
            Display Name
          </label>
          <input
            type="text"
        ...
        <button
          ...
          disabled={saving}
          className="px-4 py-2 rounded-lg bg-electric/15 border border-electric/25 text-electric text-xs font-semibold hover:bg-electric/25 transition-all flex items-center gap-1.5 disabled:opacity-50"
        >
          {saving ? (
      ...
      <div className="glass rounded-xl p-4 space-y-2">
        ...
        <div className="flex items-center gap-3">
          <code className="text-lg font-mono text-neon font-bold tracking-widest">
            {profile.invite_code}
          </code>
          <button
            onClick={copyInvite}
          ...
          >
            {copied ? (
              ...
              <Copy className="w-4 h-4 text-mercury/50" />
            )}
          </button>
        </div>
        <p className="text-[10px] text-mercury/50">
          {profile.invites_remaining} invites remaining • Share your code to let
          friends join
        </p>
      </div>
      ...
      <div className="glass rounded-xl p-4 space-y-3">
        ...
        <div className="space-y-2.5">
          <div className="flex items-center justify-between">
            ...
            >
              <div
                className={`w-4 h-4 rounded-full bg-white shadow transition-transform ${pushEnabled ? "translate-x-5" : "translate-x-0.5"}`}
              />
            </button>
          </div>
          {pushEnabled && (
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[9px] text-mercury/50">
                Summary of picks + results each morning
              </p>
            </div>
            <button
            ...
            >
              <div
                className={`w-4 h-4 rounded-full bg-white shadow transition-transform ${emailRecap ? "translate-x-5" : "translate-x-0.5"}`}
              />
            </button>
          </div>
          <div>
        ...
        <button
          ...
          disabled={saving}
          className="px-4 py-2 rounded-lg bg-amber/15 border border-amber/25 text-amber text-xs font-semibold hover:bg-amber/25 transition-all flex items-center gap-1.5 disabled:opacity-50"
        >
          {saving ? (
            <Loader2 className="w-3 h-3 animate-spin" />
          ) : saved ? (
      ...
      <div className="glass rounded-xl p-4 space-y-2">
        ...
        {profile.devices?.length > 0 && (
          <p className="text-[10px] text-mercury/50">
            Active on {profile.devices.length} device
            {profile.devices.length > 1 ? "s" : ""}
          </p>
        )}
      </div>
