  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-3 gap-2 text-center">
        <div>
          <p
            className={`text-2xl font-bold font-mono tabular-nums ${final >= 0 ? "text-neon" : "text-danger"}`}
          >
            {final >= 0 ? "+" : ""}${final.toFixed(0)}
          </p>
          <p className="text-[10px] text-mercury/60 uppercase">
            Profit @ $100/bet
          </p>
        </div>
        <div>
          <p
            className={`text-2xl font-bold font-mono tabular-nums ${wr >= 55 ? "text-neon" : wr >= 50 ? "text-electric" : "text-amber"}`}
          >
            {wr.toFixed(1)}%
          </p>
      ...
      >
        ...
        <path
          ...
          fill={final >= 0 ? "rgba(0,255,136,0.12)" : "rgba(255,59,92,0.10)"}
        />
        <path
          d={path}
