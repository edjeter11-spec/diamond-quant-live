  ...
  return (
    <div className="max-w-6xl mx-auto space-y-3">
      ...
      <div className="glass rounded-xl px-4 py-3 flex items-center justify-between border border-gold/20">
        ...
        <div className="flex items-center gap-2">
          <span className="hidden sm:block text-[10px] text-mercury/50 font-mono">
            Auto-refresh 60s
          </span>
          <button
      ...
      {arbs.length === 0 ? (
        <div className="glass rounded-xl p-10 text-center border border-slate/20">
          ...
          <p className="text-sm text-silver font-semibold mb-1">
            No arbs right now — books are sharp
          </p>
          <p className="text-[11px] text-mercury/60">
            We scan every market every poll. New opportunities will surface here
            automatically.
          </p>
        </div>
      ) : (
            ...
            <table className="w-full text-xs">
              <thead className="bg-bunker/80 border-b border-slate/20">
                <tr className="text-left">
                  <th className="px-3 py-2 font-semibold text-mercury uppercase tracking-wider text-[10px]">
                    Game
                  </th>
                  <th className="px-3 py-2 font-semibold text-mercury uppercase tracking-wider text-[10px]">
                    Side A
                  ...
                  <th className="px-3 py-2 font-semibold text-mercury uppercase tracking-wider text-[10px] text-right">
                    Stake (per $100)
                  </th>
                  <th className="px-3 py-2 font-semibold text-gold uppercase tracking-wider text-[10px] text-right">
                    Locked Profit
                  </th>
                </tr>
              </thead>
                ...
                {arbs.map((arb, i) => (
                  <tr key={i} className="hover:bg-gold/5 transition-colors">
                    <td className="px-3 py-2.5">
                      <p className="text-silver font-medium truncate max-w-[220px]">
                        {arb.game}
                      </p>
                      <p className="text-[10px] text-mercury/50 uppercase tracking-wider">
                        {arb.type}
                      </p>
                    </td>
                    <td className="px-3 py-2.5">
                    ...
                    <td className="px-3 py-2.5 text-right">
                      <p className="text-gold font-mono font-bold text-sm">
                        ${arb.profit.toFixed(2)}
                      </p>
                      <p className="text-[10px] text-gold/60 font-mono">
                        +{arb.profit.toFixed(2)}%
                      </p>
                    </td>
                  </tr>
              ...
              <div key={i} className="p-3 space-y-2">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="text-xs text-silver font-medium truncate">
                      {arb.game}
                    </p>
                    <p className="text-[9px] text-mercury/50 uppercase tracking-wider">
                      {arb.type}
                    </p>
                  </div>
                  <div className="text-right flex-shrink-0">
                    ...
                    <p className="text-[9px] text-gold/60 font-mono">
                      +{arb.profit.toFixed(2)}%
                    </p>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-2 text-[11px]">
                  <div className="rounded-lg bg-bunker/60 border border-slate/10 p-2">
                    ...
                    <p className="text-[10px] text-mercury/70 font-mono mt-1">
                      ${arb.stake1.toFixed(2)}
                    </p>
                  </div>
                  <div className="rounded-lg bg-bunker/60 border border-slate/10 p-2">
      ...
      <p className="text-[10px] text-mercury/40 text-center font-mono px-4">
        Stake split assumes a $100 total wager. Profit shown is guaranteed
        regardless of outcome — check book limits & line availability before
        placing.
      </p>
    </div>
  );
