...
export default function GhostBots() {
  ...
  useEffect(() => {
    ...
    const needsPicks = system.ghosts.some(
      (g) => g.picks.filter((p) => p.date === today).length === 0,
    );
    if (needsPicks) {
    ...
    <div className="space-y-3">
      ...
      <div className="glass rounded-xl p-4">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-[10px] text-mercury/50 ml-auto">
            3 strategies competing
          </span>
        </div>
        <p className="text-xs text-mercury">
          Three shadow bots run different strategies in parallel. Whichever has
          the best ROI after enough data automatically becomes the{" "}
          <span className="text-neon font-semibold">Live Bot</span> that powers
          the main Board picks.
        </p>
        ...
        {system.swapHistory.length > 0 && (
          <div className="mt-2 flex items-center gap-2 px-2 py-1.5 rounded bg-neon/5 border border-neon/15">
            ...
            <p className="text-[10px] text-neon">
              Last swap:{" "}
              {system.swapHistory[system.swapHistory.length - 1].reason}
            </p>
          </div>
        )}
      </div>
          ...
          {sortedGhosts.map((ghost) => {
            ...
            return (
              <div key={ghost.id} className="contents">
                <div
                  className={`bg-bunker/50 px-3 py-2.5 flex items-center gap-1.5 ${ghost.isLive ? "border-l-2 border-neon" : ""}`}
                >
                  <Icon className={`w-3.5 h-3.5 ${color}`} />
                  <div>
                    <span className="text-xs font-semibold text-silver">
                      {ghost.name}
                    </span>
                    {ghost.isLive && (
                      <Crown className="w-3 h-3 text-neon inline ml-1" />
                    )}
                  </div>
                </div>
                <div className="bg-bunker/50 px-3 py-2.5 text-center">
                  <span className="text-xs font-mono text-silver">
                    {ghost.wins}W-{ghost.losses}L
                  </span>
                </div>
                <div className="bg-bunker/50 px-3 py-2.5 text-center">
                  <span
                    className={`text-xs font-mono font-bold ${ghost.roi >= 0 ? "text-neon" : "text-danger"}`}
                  >
                    {ghost.roi >= 0 ? "+" : ""}
                    {ghost.roi.toFixed(1)}%
                  </span>
                ...
                <div className="bg-bunker/50 px-3 py-2.5 text-center">
                  <span
                    className={`text-xs font-mono ${ghost.bankroll >= 5000 ? "text-neon" : "text-danger"}`}
                  >
      ...
      {sortedGhosts.map((ghost) => {
        ...
        return (
          <div
            key={ghost.id}
            className={`glass rounded-xl overflow-hidden ${ghost.isLive ? "border border-neon/20" : ""}`}
          >
            <button
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
                  ...
                  {ghost.isLive && (
                    <span className="text-[9px] px-1.5 py-0.5 bg-neon/15 text-neon rounded font-bold">
                      LIVE
                    </span>
                  )}
                </div>
              ...
              <div className="text-right flex-shrink-0 mr-2">
                <p
                  className={`text-sm font-mono font-bold ${ghost.profit >= 0 ? "text-neon" : "text-danger"}`}
                >
                  {ghost.profit >= 0 ? "+" : ""}${ghost.profit.toFixed(0)}
                </p>
                <p className="text-[9px] text-mercury">
                  min {strategy?.minEdge}% edge
                </p>
              </div>
              <ChevronDown
                className={`w-4 h-4 text-mercury/40 transition-transform ${isExpanded ? "rotate-180" : ""}`}
              />
            </button>
            ...
            {isExpanded && (
              <div className="px-3 sm:px-4 pb-3 animate-slide-up space-y-3">
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <div className="text-center p-2 rounded bg-gunmetal/40">
                    <p className="text-sm font-bold font-mono text-silver">
                      {strategy?.maxBetsPerDay}
                    </p>
                    <p className="text-[8px] text-mercury uppercase">Max/Day</p>
                  </div>
                  <div className="text-center p-2 rounded bg-gunmetal/40">
                    <p className="text-sm font-bold font-mono text-silver">
                      {((strategy?.kellyFraction ?? 0.25) * 100).toFixed(0)}%
                    </p>
                  ...
                  <div>
                    ...
                    <div className="flex gap-1">
                      {recentPicks.map((p, i) => (
                        <div
                          key={i}
                          className={`flex-1 h-5 rounded flex items-center justify-center text-[8px] font-bold ${
                            p.result === "win"
...
function GhostPickRow({
  ...
  return (
    <div className="flex items-center gap-2 px-2 py-1.5 rounded bg-gunmetal/30">
      ...
      {pick.result !== "pending" && (
        <span
          className={`text-[10px] font-mono font-bold ${resultStyles[pick.result]}`}
        >
