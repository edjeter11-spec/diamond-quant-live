...
export default function MatchupInsights({
  ...
  const insights = useMemo(() => {
    ...
    const l5Avg =
      last5.length > 0
        ? last5.reduce((s, g) => s + getStat(g, market), 0) / last5.length
        : 0;
    const l3Avg =
      last3.length > 0
        ? last3.reduce((s, g) => s + getStat(g, market), 0) / last3.length
        : 0;
    ...
    const homeGames = log.filter(
      (g) => g.isHome === true || g.venue?.toLowerCase?.().includes("home"),
    );
    const awayGames = log.filter((g) => g.isHome === false);
    const homeAvg =
      homeGames.length > 0
        ? homeGames.reduce((s, g) => s + getStat(g, market), 0) /
          homeGames.length
        : null;
    const awayAvg =
      awayGames.length > 0
        ? awayGames.reduce((s, g) => s + getStat(g, market), 0) /
          awayGames.length
        : null;
    ...
    const trendPct =
      seasonReference > 0
        ? ((l3Avg - seasonReference) / seasonReference) * 100
        : 0;
    ...
    if (hitL10 >= 7)
      advice.push(
        `Hot streak: cleared this line in ${hitL10}/${valueList.length} of last games`,
      );
    else if (hitL10 <= 3 && valueList.length >= 6)
      advice.push(
        `Cold vs this line: cleared ${hitL10}/${valueList.length} recently`,
      );

    // Trend
    if (trendPct > 15)
      advice.push(`Recent form ↑ ${trendPct.toFixed(0)}% vs season — hot hand`);
    else if (trendPct < -15)
      advice.push(
        `Recent form ↓ ${Math.abs(trendPct).toFixed(0)}% vs season — regression spot`,
      );
    ...
    if (
      homeAvg != null &&
      awayAvg != null &&
      Math.abs(homeAvg - awayAvg) > line * 0.15
    ) {
      const strongerAtHome = homeAvg > awayAvg;
      if (isHomeTonight === strongerAtHome) {
        advice.push(
          `Plays well at ${strongerAtHome ? "home" : "away"}: ${(strongerAtHome ? homeAvg : awayAvg).toFixed(1)} avg (+${Math.abs(homeAvg - awayAvg).toFixed(1)} vs other side)`,
        );
      } else if (isHomeTonight !== undefined) {
        advice.push(
          `Weaker ${isHomeTonight ? "at home" : "on road"}: ${(isHomeTonight ? homeAvg : awayAvg).toFixed(1)} avg`,
        );
      }
    }
    ...
    if (vsOpponent && vsOpponent.games >= 2) {
      ...
      advice.push(
        `vs this opponent (${vsOpponent.games}g): averages ${vsOpponent.avgStat.toFixed(1)} — ${vsLine ? "trends OVER" : "trends UNDER"}`,
      );
    }
    ...
    const variance =
      valueList.length > 1
        ? Math.sqrt(
            valueList.reduce((s, v) => s + Math.pow(v - l10Avg, 2), 0) /
              valueList.length,
          )
        : 0;
    if (variance > l10Avg * 0.4 && l10Avg > 0) {
      advice.push(
        `Volatile — swings ±${variance.toFixed(1)} around ${l10Avg.toFixed(1)} avg. Risky either side.`,
      );
    } else if (variance < l10Avg * 0.15 && l10Avg > 0) {
      advice.push(
        `Consistent producer (low variance) — projection is reliable`,
      );
    }
    ...
    return {
      ...
      hitRateL5:
        last5.length > 0 ? Math.round((hitL5 / last5.length) * 100) : 0,
  ...
  return (
    <div className="rounded-lg bg-gunmetal/30 border border-electric/15 p-3 space-y-3">
      <div className="flex items-center gap-2">
        ...
        <p className="text-[11px] text-electric uppercase tracking-wider font-bold">
          Matchup Insights
        </p>
      </div>
      ...
      <div className="grid grid-cols-4 gap-1.5 text-center">
        <div className="p-1.5 rounded bg-bunker/60 border border-slate/20">
          ...
          <p
            className={`text-sm font-bold font-mono ${insights.l3Avg >= line ? "text-neon" : "text-mercury"}`}
          >
            {insights.l3Avg}
          </p>
        </div>
        <div className="p-1.5 rounded bg-bunker/60 border border-slate/20">
          ...
          <p
            className={`text-sm font-bold font-mono ${insights.l5Avg >= line ? "text-neon" : "text-mercury"}`}
          >
            {insights.l5Avg}
          </p>
        </div>
        <div className="p-1.5 rounded bg-bunker/60 border border-slate/20">
          ...
          <p
            className={`text-sm font-bold font-mono ${insights.l10Avg >= line ? "text-neon" : "text-mercury"}`}
          >
            {insights.l10Avg}
          </p>
        </div>
        ...
        <div className="flex items-center gap-1 px-2 py-1 rounded-md bg-bunker/60 border border-slate/20">
          ...
          <span
            className={`text-[11px] font-bold font-mono ${
              insights.hitRateL10 >= 70
                ? "text-neon"
                : insights.hitRateL10 >= 50
                  ? "text-amber"
                  : "text-danger"
            }`}
          >
      ...
      {insights.advice.length > 0 && (
        <div className="pt-2 border-t border-slate/15 space-y-1">
          <div className="flex items-center gap-1.5 mb-1">
            ...
            <p className="text-[10px] text-gold uppercase tracking-wider font-bold">
              Advice
            </p>
          </div>
          <ul className="space-y-1">
            {insights.advice.slice(0, 5).map((note, i) => (
              <li
                key={i}
                className="flex items-start gap-1.5 text-[11px] text-silver"
              >
