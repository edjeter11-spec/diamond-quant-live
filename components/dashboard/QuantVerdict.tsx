...
export default function QuantVerdict({
  game,
  analysis,
  onPlaceBet,
}: QuantVerdictProps) {
    ...
    return (
      <div className="glass rounded-xl p-6">
        ...
        <p className="text-mercury text-sm">
          Select a game to see the model's analysis
        </p>
      </div>
    );
  }
  ...
  const conf =
    confidenceColors[analysis.confidence] ?? confidenceColors.NO_EDGE;
  ...
  return (
    <div
      className={`glass rounded-xl overflow-hidden ${analysis.confidence === "HIGH" ? "glow-neon" : ""}`}
    >
      {/* Header */}
      <div className={`px-4 py-3 border-b border-slate/50 ${conf.bg}`}>
        <div className="flex items-center justify-between">
          ...
          <span
            className={`px-2 py-0.5 rounded text-xs font-bold uppercase ${conf.bg} ${conf.border} ${conf.text} border`}
          >
      ...
      <div className="p-4 sm:p-5">
        <div className="text-center mb-4 sm:mb-6">
          <p className="text-xs sm:text-sm text-mercury mb-1">
            {game.awayTeam} @ {game.homeTeam}
          </p>
          <p className={`text-xl sm:text-2xl font-bold ${conf.text} mb-1`}>
            {analysis.pick}
          </p>
          <p className="text-[11px] sm:text-xs text-mercury">
            Best price @ {analysis.bookmaker}
          </p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 sm:gap-4 mb-4 sm:mb-5">
          ...
          <div className="text-center p-2 sm:p-3 rounded-lg bg-gunmetal/50">
            ...
            <p className="text-[9px] sm:text-[10px] text-mercury uppercase tracking-wider mt-0.5">
              Win Prob
            </p>
          </div>
          ...
          <div className="text-center p-2 sm:p-3 rounded-lg bg-gunmetal/50">
            ...
            <p
              className={`text-lg sm:text-2xl font-bold font-mono ${evPercentage > 0 ? "text-neon" : "text-danger"}`}
            >
              {evPercentage > 0 ? "+" : ""}
        ...
        <div className="flex items-center justify-between px-3 sm:px-4 py-2 sm:py-2.5 rounded-lg bg-gunmetal/30 mb-3 sm:mb-4">
          <div className="text-center">
            ...
            <p className="text-lg font-mono font-bold text-silver">
              {formatOdds(marketOdds)}
            </p>
          </div>
          <div className="flex flex-col items-center">
            <span
              className={`text-xs ${evPercentage > 0 ? "text-neon" : "text-danger"}`}
            >
          ...
          <div className="text-center">
            ...
            <p className="text-lg font-mono font-bold text-electric">
              {formatOdds(fairOdds)}
            </p>
          </div>
        </div>
        ...
        >
          <span className="text-sm text-mercury font-medium">
            Why this bet?
          </span>
          {showReasoning ? (
        ...
        {showReasoning && (
          <div className="mt-2 px-4 py-3 rounded-lg bg-gunmetal/20 space-y-2 animate-slide-up">
            ...
            {reasoning.length === 0 && (
              <p className="text-sm text-mercury/60 italic">
                No reasoning available — pick is based on raw market mispricing.
              </p>
            )}
          </div>
        )}
