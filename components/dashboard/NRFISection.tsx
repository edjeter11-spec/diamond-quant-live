...
export default function NRFISection({ sport }: { sport: "mlb" | "nba" }) {
  ...
  const totalNRFI = recommendations.filter((g) =>
    g.recommendation.includes("NRFI"),
  ).length;
  const totalYRFI = recommendations.filter((g) =>
    g.recommendation.includes("YRFI"),
  ).length;
  ...
  return (
    <div className="glass rounded-xl overflow-hidden border border-electric/20">
      ...
      <div className="px-4 py-3 bg-gradient-to-r from-electric/10 to-transparent border-b border-electric/15 flex items-center gap-2">
        ...
        <div className="flex-1">
          ...
          <p className="text-[10px] text-mercury/60">
            {totalNRFI} NRFI · {totalYRFI} YRFI ·{" "}
            {upcoming.length - totalNRFI - totalYRFI} pass
          </p>
        </div>
        ...
        {visible.map((g) => {
          ...
          return (
            <div key={g.gameId}>
              ...
              >
                <span
                  className={`w-7 h-7 rounded-lg flex items-center justify-center text-[11px] font-bold border ${grade} flex-shrink-0`}
                >
                  {g.nrfiGrade}
                </span>
                ...
                <div className="text-right flex-shrink-0">
                  <p className={`text-sm font-mono font-bold ${probColor}`}>
                    {g.nrfiProb}%
                  </p>
              ...
              {isExpanded && (
                <div className="px-4 pb-3 space-y-3 animate-slide-up">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-[9px] text-neon font-bold w-16">
                      NRFI {g.nrfiProb}%
                    </span>
                    <div className="flex-1 h-2 bg-gunmetal rounded-full overflow-hidden flex">
                      <div
                        className="h-full bg-neon"
                        style={{ width: `${g.nrfiProb}%` }}
                      />
                      <div
                        className="h-full bg-danger"
                        style={{ width: `${g.yrfiProb}%` }}
                      />
                    </div>
                    <span className="text-[9px] text-danger font-bold w-16 text-right">
                      {g.yrfiProb}% YRFI
                    </span>
                  </div>
                    ...
                    <div className="rounded-lg bg-gunmetal/20 p-2.5 space-y-0.5">
                      {g.factors.map((f, i) => (
                        <p key={i} className="text-[10px] text-mercury">
                          <span className="text-electric mr-1">›</span>
                          {f}
                        </p>
                      ))}
                    </div>
      ...
      <div className="px-4 py-2 bg-gunmetal/10 border-t border-slate/15 flex items-center justify-between">
        <span className="text-[9px] text-mercury/50">
          Auto-graded after 1st inning · history tracked
        </span>
        <span className="text-[9px] text-mercury/50">
          Engine: ERA + WHIP + K/9 + park
        </span>
      </div>
    </div>
  );
  ...
  return (
    <div className="rounded-lg bg-gunmetal/30 p-2.5">
      <div className="flex items-center gap-1.5 mb-1.5">
        ...
        <p className="text-[11px] font-semibold text-silver truncate">
          {pitcher.name}
        </p>
      </div>
