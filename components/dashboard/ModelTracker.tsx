  ...
  if (stats.total === 0 && stats.pending === 0) {
    return (
      <div className="glass rounded-xl p-6 sm:p-8 text-center">
        ...
        <h3 className="text-base font-bold text-silver mb-1">
          Model Accuracy Tracker
        </h3>
        <p className="text-sm text-mercury">
          Start logging bets to track the model's performance over time.
        </p>
        <p className="text-xs text-mercury/50 mt-2">
          Use "Log a Bet" on the Bank tab, or tap picks on the dashboard to log
          them.
        </p>
      </div>
    );
  ...
  return (
    <div className="glass rounded-xl overflow-hidden">
      <div className="px-4 py-3 border-b border-slate/50 flex items-center gap-2">
        ...
        {stats.total > 0 && (
          <span className="ml-auto text-xs font-mono text-mercury">
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="flex items-center justify-between px-1">
          ...
          {stats.streak > 0 && (
            <div className="flex items-center gap-1">
              <Flame
                className={`w-3.5 h-3.5 ${stats.streakType === "W" ? "text-neon" : "text-danger"}`}
              />
              <span
                className={`text-xs font-bold ${stats.streakType === "W" ? "text-neon" : "text-danger"}`}
              >
                {stats.streak}
        ...
        {stats.recent10.length > 0 && (
          <div>
            <p className="text-[10px] text-mercury uppercase tracking-wider mb-1.5">
              Last {stats.recent10.length}
            </p>
          ...
          <div>
            ...
            <div className="space-y-1">
              {Object.entries(stats.byMarket).map(([market, data]) => (
                <div
                  key={market}
                  className="flex items-center justify-between px-2 py-1.5 rounded bg-gunmetal/20"
                >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span
                      className={`text-[10px] font-mono font-semibold ${data.profit >= 0 ? "text-neon" : "text-danger"}`}
                    >
        ...
        {stats.pending > 0 && (
          <div className="flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-amber/5 border border-amber/15">
            ...
            <span className="text-xs text-amber">
              {stats.pending} pending {stats.pending === 1 ? "bet" : "bets"} —
              settle them on the Bank tab
            </span>
          </div>
        )}
      </div>
