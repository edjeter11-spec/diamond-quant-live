...
export default function BrainViz() {
  ...
  const [activeView, setActiveView] = useState<
    "thoughts" | "pitchers" | "parks" | "matchups" | "timeline"
  >("thoughts");
  ...
  return (
    <div className="glass rounded-xl overflow-hidden border border-purple/15">
      ...
      <div className="px-4 py-4 bg-gradient-to-r from-purple/10 via-electric/5 to-neon/5 border-b border-purple/15">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-[10px] text-mercury/60 font-mono">
              {summary.version} • Epoch {summary.epoch} •{" "}
              {summary.totalGamesProcessed.toLocaleString()} games learned
            </p>
          </div>
          <div className="text-right">
            <p
              className={`text-lg font-bold font-mono ${summary.overallWinRate > 52 ? "text-neon" : "text-silver"}`}
            >
        ...
        <div className="grid grid-cols-4 gap-2 mt-3">
          <KnowledgeStat
            icon={Database}
            label="Seasons"
            value={brain.trainedSeasons?.join(", ") ?? "?"}
            color="text-purple"
          />
          <KnowledgeStat
            icon={Target}
          ...
          <KnowledgeStat
            icon={Shield}
            label="Parks"
            value={String(parkCount)}
            color="text-neon"
          />
          <KnowledgeStat
            icon={Activity}
            label="Matchups"
            value={String(matchupCount)}
            color="text-amber"
          />
        </div>
      </div>
      ...
      <div className="max-h-[400px] overflow-y-auto">
        {activeView === "thoughts" && (
          <ThoughtsView thoughts={thoughts} brain={brain} summary={summary} />
        )}
    ...
    <div className="p-4 space-y-3">
      ...
      <div className="rounded-lg bg-gunmetal/20 p-3">
        ...
        <div className="space-y-1.5">
          {Object.entries(brain.weights)
            .sort(([, a], [, b]) => (b as number) - (a as number))
            .slice(0, 4)
            .map(([key, val], i) => (
              <div key={key} className="flex items-center gap-2">
                <span className="text-[10px] text-mercury w-16 capitalize">
                  {key}
                </span>
                <div className="flex-1 h-2 bg-gunmetal rounded-full overflow-hidden">
                  <div
                    className="h-full bg-gradient-to-r from-purple to-electric rounded-full transition-all"
                    style={{ width: `${(val as number) * 400}%` }}
                  />
                </div>
                <span className="text-[10px] font-mono text-silver w-10 text-right">
      ...
      {/* Market intelligence */}
      <div className="rounded-lg bg-gunmetal/20 p-3">
        <p className="text-[9px] text-mercury uppercase tracking-wider mb-2 font-semibold">
          Market Confidence
        </p>
        <div className="grid grid-cols-2 gap-2">
          {Object.entries(brain.markets)
            .filter(([, m]) => (m as any).totalBets > 0)
            .map(([key, market]: [string, any]) => (
              <div key={key} className="p-2 rounded bg-bunker/50">
                <p className="text-[10px] text-silver capitalize font-semibold">
                  {key.replace("_", " ")}
                </p>
                <p
                  className={`text-sm font-mono font-bold ${market.winRate > 52 ? "text-neon" : market.winRate < 48 ? "text-danger" : "text-silver"}`}
                >
                  {market.winRate.toFixed(1)}%
                </p>
                <p className="text-[8px] text-mercury">
                  {market.wins}W-{market.losses}L • min{" "}
                  {market.dynamicThreshold.toFixed(1)}% edge
                </p>
              </div>
            ))}
        </div>
...
function PitchersView({ brain }: { brain: BrainState }) {
  ...
  if (pitchers.length === 0)
    return (
      <div className="p-6 text-center text-mercury text-sm">
        No pitcher data yet — will build as games settle
      </div>
    );
    ...
    <div className="divide-y divide-slate/10">
      {pitchers.slice(0, 30).map((p: any, i: number) => (
        <div key={i} className="px-4 py-2.5 flex items-center gap-2">
          <div
            className={`w-1.5 h-8 rounded-full flex-shrink-0 ${p.winRate > 55 ? "bg-neon" : p.winRate > 45 ? "bg-electric" : "bg-danger"}`}
          />
          <div className="flex-1 min-w-0">
            <p className="text-xs font-medium text-silver truncate">{p.name}</p>
            <p className="text-[9px] text-mercury/60">
              {p.gamesTracked} games tracked • NRFI: {p.nrfiRate}%
            </p>
          </div>
          <div className="text-right flex-shrink-0">
            <p
              className={`text-sm font-mono font-bold ${p.winRate > 55 ? "text-neon" : p.winRate < 45 ? "text-danger" : "text-silver"}`}
            >
              {p.winRate}%
            </p>
            <p className="text-[8px] text-mercury">
...
function ParksView({ brain }: { brain: BrainState }) {
  const parks = Object.entries(brain.parkMemory ?? {})
    ...
    .sort(
      ([, a]: [string, any], [, b]: [string, any]) => b.avgRuns - a.avgRuns,
    );
  ...
  return (
    <div className="divide-y divide-slate/10">
      {parks.map(([name, p]: [string, any], i: number) => {
        const homeRate =
          p.games > 0 ? Math.round((p.homeWins / p.games) * 100) : 50;
        return (
          <div key={i} className="px-4 py-2.5 flex items-center gap-2">
            <div
              className={`w-1.5 h-8 rounded-full flex-shrink-0 ${p.avgRuns > 9 ? "bg-danger" : p.avgRuns > 8 ? "bg-amber" : "bg-neon"}`}
            />
            <div className="flex-1 min-w-0">
              <p className="text-xs font-medium text-silver truncate">{name}</p>
              <p className="text-[9px] text-mercury/60">
                {p.games} games • Home wins: {homeRate}% • NRFI:{" "}
                {Math.round(p.nrfiRate)}%
              </p>
            </div>
            <div className="text-right flex-shrink-0">
              <p
                className={`text-sm font-mono font-bold ${p.avgRuns > 9 ? "text-danger" : p.avgRuns < 8 ? "text-neon" : "text-silver"}`}
              >
                {p.avgRuns.toFixed(1)}
              </p>
  ...
  return (
    <div className="divide-y divide-slate/10">
      {matchups.slice(0, 30).map(([key, m]: [string, any], i: number) => {
        ...
        const homeRate =
          m.games > 0 ? Math.round((m.homeWins / m.games) * 100) : 50;
        return (
          <div key={i} className="px-4 py-2.5 flex items-center gap-2">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[9px] text-mercury/60">
                {m.games} games • Home: {m.homeWins}W-{m.games - m.homeWins}L
              </p>
            </div>
            <div className="w-20 h-2 bg-gunmetal rounded-full overflow-hidden flex">
              ...
              <div
                ...
                style={{ width: `${100 - homeRate}%` }}
              />
            </div>
            <span className="text-[10px] font-mono text-mercury w-10 text-right">
              {homeRate}%
            </span>
          </div>
        );
...
function TimelineView({ brain }: { brain: BrainState }) {
  ...
  return (
    <div className="p-3 space-y-1">
      {logs.map((log, i) => {
        ...
        return (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[11px] text-mercury leading-relaxed">
                {log.message}
              </p>
              <p className="text-[8px] text-mercury/40">
      ...
      {logs.length === 0 && (
        <p className="text-center text-mercury/40 text-sm py-4">
          No events yet
        </p>
      )}
    </div>
...
function generateThoughts(brain: BrainState): string[] {
  ...
  // Weight insights
  const topWeight = Object.entries(w).sort(
  ...
  // Market insights
  const bestMarket = Object.entries(brain.markets)
    .filter(([, m]) => (m as any).totalBets > 10)
    .sort(([, a]: any, [, b]: any) => b.winRate - a.winRate)[0];
  if (bestMarket) {
    const m = bestMarket[1] as any;
    thoughts.push(
      `My strongest market is ${bestMarket[0]}: ${m.winRate.toFixed(1)}% accuracy over ${m.totalBets} predictions. I've ${m.winRate > 53 ? "earned confidence here and lowered my minimum edge to " + m.dynamicThreshold.toFixed(1) + "%." : "set a higher " + m.dynamicThreshold.toFixed(1) + "% minimum edge because I need more certainty before betting."}`,
    );
  }

  // Pitcher insights
  const topPitchers = Object.values(brain.pitcherMemory ?? {})
  ...
  if (topPitchers.length >= 3) {
    ...
    thoughts.push(
      `I've tracked ${topPitchers.length} pitchers with 10+ games. ${best.name} has the highest win rate I've seen (${best.winRate}% in ${best.gamesTracked} games). ${worst.name} is at ${worst.winRate}% — when I see them start, I lean the other way.`,
    );
  }

  // Park insights
  const parks = Object.entries(brain.parkMemory ?? {}).filter(
    ([, p]: any) => p.games >= 20,
  );
  if (parks.length > 0) {
    const highest = parks.sort(
      ([, a]: any, [, b]: any) => b.avgRuns - a.avgRuns,
    )[0];
    const lowest = parks.sort(
      ([, a]: any, [, b]: any) => a.avgRuns - b.avgRuns,
    )[0];
    thoughts.push(
      `${highest[0]} averages ${(highest[1] as any).avgRuns.toFixed(1)} runs/game — I boost Overs there. ${lowest[0]} is the opposite at ${(lowest[1] as any).avgRuns.toFixed(1)} — pitcher-friendly, I favor Unders and NRFI.`,
    );
  }

  // NRFI insight
  const nrfiParks = Object.entries(brain.parkMemory ?? {}).filter(
    ([, p]: any) => p.games >= 20 && p.nrfiRate > 75,
  );
  if (nrfiParks.length > 0) {
    thoughts.push(
      `Best NRFI parks I've found: ${nrfiParks
        .slice(0, 3)
        .map(([name, p]: any) => `${name} (${Math.round(p.nrfiRate)}%)`)
        .join(", ")}. I weight these heavily in first-inning analysis.`,
    );
  }

  // Learning speed
  thoughts.push(
    `I've processed ${brain.totalGamesProcessed.toLocaleString()} games across ${brain.trainedSeasons?.length ?? 0} seasons. Every game that finishes today feeds back into my weights. My learning rate is ${brain.learningRate} — slow and stable to avoid overreacting to individual results.`,
  );
  ...
  if (recentGames.length >= 5) {
    const recentCorrect = recentGames.filter((g) => {
      const right =
        (g.actual === "home" && g.prediction > 0.5) ||
        (g.actual === "away" && g.prediction < 0.5);
      return right;
    }).length;
    thoughts.push(
