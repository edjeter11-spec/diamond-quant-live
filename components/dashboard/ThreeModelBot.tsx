...
export default function ThreeModelBot() {
  ...
  async function fetchAnalysis() {
    ...
    try {
      const url =
        currentSport === "nba" ? "/api/nba-analysis" : "/api/bot-analysis";
    ...
    return (
      <div className="glass rounded-xl p-8 text-center">
        ...
        <p className="text-sm text-silver font-medium">
          Running 3 models on every game...
        </p>
        <p className="text-[10px] text-mercury/60 mt-1">
          Net Rating + Market consensus + Power ranking analysis
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-3">
      ...
      <div className="glass rounded-xl p-4 border border-electric/15">
        <div className="flex items-center gap-2 mb-2">
          ...
          <button
            ...
            className="ml-auto p-1.5 hover:bg-gunmetal/30 rounded transition-colors"
          >
            <RefreshCw
        ...
        <p className="text-[10px] text-mercury/50 mt-1">
          When all 3 agree → HIGH confidence. When they disagree → that's where
          the edge is.
        </p>
      </div>
      ...
      {analyses.length === 0 && !error && (
        <div className="glass rounded-xl p-6 text-center">
          ...
          <p className="text-[10px] text-mercury/50 mt-1">
            Check back when today's slate has odds posted
          </p>
        </div>
      )}
          ...
          >
            {game.consensus.confidence === "NO_PLAY" && (
              <div className="flex items-center gap-2 px-3 sm:px-4 py-1.5 bg-mercury/5 border-b border-slate/20">
                <AlertTriangle className="w-3.5 h-3.5 text-mercury/70 flex-shrink-0" />
                <p className="text-[10px] sm:text-[11px] font-semibold text-mercury/80 uppercase tracking-wide">
                  Model says pass on this one
                </p>
              </div>
            )}
            {/* Game Row */}
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
                  ...
                  {!game.consensus.modelsAgree && (
                    <span className="text-[8px] px-1 py-0.5 rounded bg-amber/10 text-amber font-bold flex-shrink-0">
                      SPLIT
                    </span>
                  )}
                </div>
              ...
              <div className="hidden sm:flex items-center gap-1 flex-shrink-0">
                <span
                  className="text-[9px] font-mono text-purple"
                  title={config.model1Label}
                >
                  {(game.pitcherModel.homeWinProb * 100).toFixed(0)}%
                </span>
                <span className="text-mercury/30">|</span>
                <span
                  className="text-[9px] font-mono text-electric"
                  title="Market Model"
                >
                  {(game.marketModel.homeWinProb * 100).toFixed(0)}%
                </span>
                <span className="text-mercury/30">|</span>
                <span
                  className="text-[9px] font-mono text-neon"
                  title="Trend Model"
                >
                  {(game.trendModel.homeWinProb * 100).toFixed(0)}%
                </span>
              </div>
              ...
              {topPick && (
                <div className="text-right flex-shrink-0">
                  <p className="text-xs font-mono font-bold text-silver">
                    {formatOdds(topPick.odds)}
                  </p>
                  <p className={`text-[9px] font-bold ${conf.text}`}>
            ...
            {isExpanded && (
              <div className="px-3 sm:px-4 pb-4 animate-slide-up space-y-3">
                ...
                {/* Consensus */}
                <div
                  className={`p-3 rounded-lg ${conf.bg} border ${conf.border}`}
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className={`text-xs font-bold ${conf.text}`}>
                      CONSENSUS: {(game.consensus.homeWinProb * 100).toFixed(1)}
                      % {game.homeTeam}
                    </span>
                    <span
                      className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${conf.bg} ${conf.text}`}
                    >
                      {game.consensus.confidence}
                    </span>
                  </div>
                ...
                <div className="rounded-lg bg-gunmetal/20 p-3">
                  ...
                  <div className="space-y-0.5 max-h-40 overflow-y-auto">
                    {[
                      ...game.pitcherModel.factors,
                      ...game.marketModel.factors,
                      ...game.trendModel.factors,
                    ].map((f, i) => (
                      <p
                        key={i}
                        className="text-[11px] text-mercury flex gap-1"
                      >
                ...
                {game.consensus.confidence === "NO_PLAY" &&
                  game.picks.length > 0 && (
                    <div className="flex items-start gap-2 px-3 py-2 rounded-lg bg-mercury/5 border border-slate/20">
                      <AlertTriangle className="w-3.5 h-3.5 text-mercury/70 flex-shrink-0 mt-0.5" />
                      <p className="text-[10px] sm:text-[11px] font-semibold text-mercury/80 uppercase tracking-wide">
                        Model says pass — leans below are for context only
                      </p>
                    </div>
                  )}
  ...
  return (
    <div className="p-2 rounded-lg bg-gunmetal/30 text-center">
      ...
      <div className="w-full h-1 bg-gunmetal rounded-full mt-1 overflow-hidden">
        <div
          className={`h-full rounded-full ${color === "text-purple" ? "bg-purple/50" : color === "text-electric" ? "bg-electric/50" : "bg-neon/50"}`}
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-1 mt-1 text-[9px]">
        ...
        <span className="text-mercury">
          K/9:{" "}
          <span className="text-silver font-mono">{pitcher.k9.toFixed(1)}</span>
        </span>
        <span className="text-mercury">
          Rec: <span className="text-silver font-mono">{pitcher.record}</span>
        </span>
      </div>
      {pitcher.vsOpponent && (
        <p className="text-[9px] text-electric mt-1">
          vs opponent: {pitcher.vsOpponent.era.toFixed(2)} ERA (
          {pitcher.vsOpponent.games}G)
        </p>
      )}
      {pitcher.fatigueRisk && (
