...
export default function BankrollTracker() {
  ...
  const [inputValue, setInputValue] = useState(
    String(bankroll.startingBankroll),
  );
  ...
  const winRate =
    bankroll.totalBets > 0 && bankroll.wins + bankroll.losses > 0
    ...
    <div className="glass rounded-xl overflow-hidden">
            ...
            <button
              onClick={() => {
                ...
                const rows = betHistory.map((b: any) => [
                  ...
                  b.result ?? "pending",
                  b.payout ?? 0,
                  ((b.payout ?? 0) - (b.stake ?? 0)).toFixed(2),
                ]);
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="text-center">
          <p className="text-xs text-mercury uppercase tracking-wider mb-1">
            Current Bankroll
          </p>
          <p
            className={`text-3xl font-bold font-mono ${isProfit ? "text-neon" : "text-danger"}`}
          >
          ...
          <div className="flex items-center justify-center gap-1 mt-1">
            ...
            <span
              className={`text-sm font-mono ${isProfit ? "text-neon" : "text-danger"}`}
            >
              {isProfit ? "+" : ""}
              {profitLoss.toFixed(2)} ({bankroll.roi.toFixed(1)}% ROI)
            </span>
          </div>
          ...
          {pendingBets.length > 0 && (
            <div className="inline-flex items-center gap-1.5 mt-2 px-2.5 py-1 rounded-full bg-amber/10 border border-amber/30">
              ...
              <span className="text-[10px] font-mono text-amber">
                $
                {pendingBets.reduce((s, b) => s + (b.stake ?? 0), 0).toFixed(2)}{" "}
                at risk · {pendingBets.length} pending · auto-settles when games
                end
              </span>
            </div>
          )}
          ...
          <div>
            ...
            <div className="space-y-2">
              {pendingBets.map((bet) => (
                <div
                  key={bet.id}
                  className="px-3 py-2.5 rounded-lg bg-amber/5 border border-amber/15"
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-mercury/60">
                        {bet.game} • {bet.bookmaker} • ${bet.stake} @{" "}
                        {bet.odds > 0 ? "+" : ""}
                        {bet.odds}
                      </p>
                    </div>
                  ...
                  <div className="flex items-center gap-1.5">
                    <button
                      onClick={() =>
                        handleSettle(bet.id, "win", bet.odds, bet.stake)
                      }
                    ...
                    <button
                      onClick={() =>
                        handleSettle(bet.id, "loss", bet.odds, bet.stake)
                      }
                    ...
                    <button
                      onClick={() =>
                        handleSettle(bet.id, "push", bet.odds, bet.stake)
                      }
        ...
        {settledBets.length > 0 && (
          <div>
            <p className="text-xs text-mercury uppercase tracking-wider mb-2">
            ...
            <div className="space-y-1.5 max-h-48 overflow-y-auto">
              {settledBets
                ...
                .map((bet) => (
                  <div
                    key={bet.id}
                    className="flex items-center justify-between px-2 py-1.5 rounded bg-gunmetal/30"
                  >
                    <div className="flex-1 min-w-0">
                      <p className="text-xs text-silver truncate">{bet.pick}</p>
                      <p className="text-[10px] text-mercury/60">
                        {bet.bookmaker} • {bet.odds > 0 ? "+" : ""}
                        {bet.odds}
                      </p>
                    </div>
                    <div className="flex items-center gap-2 ml-2">
                      <span className="text-xs font-mono text-mercury">
                        ${bet.stake}
                      </span>
                      <span
                      ...
                      {bet.result === "win" && (
                        <span className="text-[10px] font-mono text-neon">
                          +${(bet.payout - bet.stake).toFixed(0)}
                        </span>
                      )}
                    </div>
                  </div>
