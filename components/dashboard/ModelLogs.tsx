...
export default function ModelLogs() {
  ...
  useEffect(() => {
    async function init() {
      ...
      const needsRetrain =
        !b.isPreTrained ||
        targetSeasons.some((s) => !b.trainedSeasons?.includes(s));
      if (needsRetrain) {
        autoTrain(b);
      }
    }
    ...
    try {
      const seasons =
        new Date().getFullYear() >= 2026
          ? [2023, 2024, 2025, 2026]
          : [2023, 2024, 2025];
      const result = await deepTrain(b, seasons, (msg) =>
        setTrainProgress(msg),
      );
      setBrain(result.finalState);
      saveBrain(result.finalState);
      setTrainProgress(
        `Deep training complete: ${result.gamesProcessed} games, ${result.pitcherGamesAnalyzed} with pitcher matchups`,
      );
    } catch (err) {
    ...
    try {
      const seasons =
        new Date().getFullYear() >= 2026
          ? [2023, 2024, 2025, 2026]
          : [2023, 2024, 2025];
      const result = await deepTrain(fresh, seasons, (msg) =>
        setTrainProgress(msg),
      );
      saveBrain(result.finalState);
      setBrain(result.finalState);
      setTrainProgress(
        `Done: ${result.gamesProcessed} games, ${result.pitcherGamesAnalyzed} pitcher matchups analyzed`,
      );
    } catch {
    ...
    <div className="glass rounded-xl overflow-hidden border border-purple/15">
      ...
      <div className="px-4 py-3 border-b border-slate/30 bg-gradient-to-r from-purple/10 to-electric/5">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-mono text-electric">
            {brain.version}
          </span>
        </div>
      ...
      <div className="grid grid-cols-4 gap-px bg-slate/10">
        <MiniStat
          label="Games"
          value={brain.totalGamesProcessed.toLocaleString()}
          color="text-silver"
        />
        <MiniStat
          label="Predictions"
          value={brain.totalPredictionsMade.toLocaleString()}
          color="text-electric"
        />
        <MiniStat
          label="Win Rate"
          value={`${summary.overallWinRate}%`}
          color={summary.overallWinRate > 52 ? "text-neon" : "text-silver"}
        />
        <MiniStat
          label="Accuracy"
          value={`${((1 - summary.avgBrier) * 100).toFixed(0)}%`}
          color="text-purple"
        />
      </div>
      ...
      {activeSection === "logs" && (
        <div className="max-h-[350px] overflow-y-auto bg-void/50 p-3 font-mono text-[11px]">
          {summary.recentLogs.length === 0 ? (
            <p className="text-mercury/40 text-center py-4">
              No logs yet — model is initializing...
            </p>
          ) : (
      ...
      {activeSection === "memory" && (
        <div className="max-h-[350px] overflow-y-auto p-3 space-y-2">
          {summary.recentGames.length === 0 ? (
            <p className="text-mercury/40 text-center text-xs py-4">
              No game memories stored yet — games will appear here as they
              finish
            </p>
          ) : (
            summary.recentGames
              ...
              .map((game, i) => {
                const wasRight =
                  (game.actual === "home" && game.prediction > 0.5) ||
                  (game.actual === "away" && game.prediction < 0.5);
                return (
                  <div
                    key={i}
                    className={`p-2.5 rounded-lg border ${wasRight ? "bg-neon/3 border-neon/10" : "bg-danger/3 border-danger/10"}`}
                  >
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-[11px] text-silver font-medium truncate">
                        {game.game}
                      </span>
                      <span className="text-[9px] text-mercury/50 ml-auto">
                        {game.date}
                      </span>
                    </div>
                    <div className="flex items-center gap-3 text-[10px] text-mercury mb-1">
                      <span>
                        Predicted: {(game.prediction * 100).toFixed(0)}% home
                      </span>
                      <span>
                        Actual: {game.actual} ({game.totalActual} runs)
                      </span>
                    ...
                    {game.lessonsLearned.length > 0 && (
                      <div className="mt-1.5 space-y-0.5">
                        {game.lessonsLearned.map((lesson, li) => (
                          <p
                            key={li}
                            className="text-[10px] text-electric/80 flex gap-1"
                          >
                            <Brain className="w-2.5 h-2.5 mt-0.5 flex-shrink-0" />{" "}
                            {lesson}
                          </p>
                        ))}
                      </div>
      ...
      {activeSection === "weights" && (
        <div className="p-3 space-y-3">
          {/* Current vs Initial */}
          <p className="text-[9px] text-mercury uppercase tracking-wider font-semibold">
            Weight Evolution (initial → current)
          </p>
            ...
            {Object.entries(summary.weights).map(([key, val]) => {
              ...
              return (
                <div key={key} className="flex items-center gap-2">
                  <span className="text-[10px] text-mercury w-16 capitalize truncate">
                    {key}
                  </span>
                  <div className="flex-1 h-3 bg-gunmetal rounded-full overflow-hidden relative">
                    {/* Initial weight marker */}
                    <div
                      className="absolute h-full w-0.5 bg-mercury/30 z-10"
                      style={{ left: `${initial * 350}%` }}
                    />
                    {/* Current weight */}
                    <div
                  ...
                  {change !== 0 && (
                    <span
                      className={`text-[9px] font-mono w-10 ${isUp ? "text-neon" : "text-danger"}`}
                    >
                      {isUp ? "+" : ""}
                      {change.toFixed(1)}
                    </span>
          ...
          {/* Market profiles */}
          <p className="text-[9px] text-mercury uppercase tracking-wider font-semibold mt-3">
            Market Intelligence
          </p>
          <div className="grid grid-cols-2 gap-2">
            {Object.entries(summary.markets)
              .filter(([_, m]) => m.totalBets > 0)
              .map(([key, market]) => (
                <div key={key} className="p-2 rounded-lg bg-gunmetal/30">
                  <p className="text-[10px] text-silver font-semibold capitalize mb-1">
                    {key.replace("_", " ")}
                  </p>
                  <div className="space-y-0.5 text-[9px]">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span
                        className={`font-mono ${market.winRate > 52 ? "text-neon" : "text-silver"}`}
                      >
          ...
          <div className="mt-2 px-3 py-2 rounded-lg bg-gunmetal/20 flex items-center gap-2">
            <CheckCircle className="w-3 h-3 text-neon flex-shrink-0" />
            <p className="text-[10px] text-mercury">
              Model auto-trains on historical data and learns from every settled
              game. No manual intervention needed.
            </p>
          </div>
        </div>
      )}
