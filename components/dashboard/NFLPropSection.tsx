      ...
      <div className="glass rounded-xl border border-electric/15">
        ...
        <div className="px-4 py-6 text-center">
          ...
          <p className="text-[10px] text-mercury/40 mt-1">
            Picks generate Thursday, Sunday, Monday
          </p>
        </div>
      </div>
    );
    ...
    <div className="glass rounded-xl overflow-hidden border border-electric/20">
      <div className="px-4 py-3 bg-gradient-to-r from-electric/10 to-transparent border-b border-electric/15 flex items-center gap-2">
        ...
        <div className="flex-1">
          ...
          <p className="text-[10px] text-mercury/60">
            {picks.length} edge picks · projector accounts for weather, rest,
            injuries, matchup defense
          </p>
        </div>
        {picks.length > 5 && (
          <button
      ...
      <div className="divide-y divide-slate/10">
        {visible.map((p, i) => {
          ...
          const sideColor =
            p.predicted_side === "over" ? "text-neon" : "text-danger";
          const evColor =
            p.ev_edge > 10
              ? "text-neon"
              : p.ev_edge > 5
          ...
          return (
            <div key={i}>
              <button
                onClick={() =>
                  setExpanded(isExp ? null : `${p.playerName}-${p.market}-${i}`)
                }
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-silver truncate">
                    {p.playerName}{" "}
                    <span className="text-[10px] text-mercury/50 ml-1">
                      {p.team}
                    </span>
                  </p>
                  <p className="text-[10px] text-mercury/70 truncate">
                    <span className={`font-bold ${sideColor}`}>
                      {p.predicted_side.toUpperCase()}
                    </span>{" "}
                    {p.line} {MARKET_LABEL[p.market] ?? p.market}
                    <span className="text-mercury/40 ml-1">
                      · {(p.predicted_prob * 100).toFixed(0)}% prob
                    </span>
                  </p>
                </div>
                <div className="text-right flex-shrink-0">
                  <p className={`text-sm font-mono font-bold ${evColor}`}>
                    +{p.ev_edge.toFixed(1)}%
                  </p>
                  <p className="text-[9px] text-mercury/50">EV</p>
                </div>
                <ChevronDown
                  className={`w-3.5 h-3.5 text-mercury/40 transition-transform ${isExp ? "rotate-180" : ""}`}
                />
              </button>
                ...
                <div className="px-4 pb-3 animate-slide-up">
                  <div className="rounded-lg bg-gunmetal/20 p-3 space-y-1.5">
                    <div className="flex items-center gap-1.5 mb-1">
                      ...
                      <p className="text-[10px] font-semibold text-silver uppercase tracking-wider">
                        Projector Factors
                      </p>
                    </div>
                    {p.factors.slice(0, 6).map((f, j) => (
                      <p
                        key={j}
                        className="text-[11px] text-mercury flex items-start gap-2"
                      >
      ...
      <div className="px-4 py-2 bg-gunmetal/10 border-t border-slate/15 flex items-center justify-between">
        <span className="text-[9px] text-mercury/50 flex items-center gap-1">
          ...
          Weather · <ThermometerSun className="w-2.5 h-2.5" />
          Rest · Matchup · Injuries
        </span>
