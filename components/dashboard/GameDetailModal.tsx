...
const fmt = (odds: number) =>
  odds === 0 ? "—" : odds > 0 ? `+${odds}` : `${odds}`;
...
export default function GameDetailModal({
  ...
  const analysis = analyses.find(
    (a: any) =>
      gameOdds &&
      (a.homeTeam === gameOdds.homeTeam ||
        a.homeAbbrev === gameOdds.homeTeam?.split(" ").pop()),
  );

  const homeTeam = gameOdds?.homeTeam ?? gameScore?.homeTeam ?? "Home";
  const awayTeam = gameOdds?.awayTeam ?? gameScore?.awayTeam ?? "Away";
  const homeAbbrev =
    gameScore?.homeAbbrev ??
    homeTeam.split(" ").pop()?.slice(0, 3).toUpperCase() ??
    "HME";
  const awayAbbrev =
    gameScore?.awayAbbrev ??
    awayTeam.split(" ").pop()?.slice(0, 3).toUpperCase() ??
    "AWY";
        ...
        <div className="px-4 sm:px-6 pt-5 pb-4 border-b border-slate/20">
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex flex-col items-center gap-2 flex-1 min-w-0">
              ...
              <p className="text-sm font-bold text-silver text-center leading-tight">
                {awayTeam}
              </p>
              {isLive && (
                <span className="text-2xl font-bold font-mono text-silver">
              ...
              {isLive ? (
                ...
                <>
                  ...
                  <p className="text-sm font-semibold text-silver">
                    {gameScore?.startTime
                      ? new Date(gameScore.startTime).toLocaleTimeString(
                          "en-US",
        ...
        <div className="px-4 sm:px-6 py-4 space-y-5">
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="border-b border-slate/25">
                        <th className="px-3 py-2 text-left text-[10px] text-mercury/60 font-medium">
                          Book
                        </th>
                    ...
                    <tbody>
                      {oddsLines.map((line: any, i: number) => (
                        <tr
                          key={i}
                          className="border-b border-slate/10 hover:bg-gunmetal/30 transition-colors"
                        >
                          <td className="px-3 py-2 font-medium text-silver">
                            {line.bookmaker}
                          </td>
                          <td
                            className={`px-2 py-2 text-center font-mono font-semibold ${
                              line.awayML === bestAwayML
                                ? "text-neon bg-neon/8 rounded"
                                : "text-mercury"
                            }`}
                          >
                            {fmt(line.awayML)}
                          ...
                          <td className="px-2 py-2 text-center font-mono text-mercury">
                            <span
                              className={
                                line.spreadPrice === bestSpreadPrice
                                  ? "text-neon font-semibold"
                                  : ""
                              }
                            >
                              {line.homeSpread > 0 ? "+" : ""}
                              {line.homeSpread} {fmt(line.spreadPrice)}
                            </span>
                          </td>
                          <td className="px-2 py-2 text-center font-mono text-mercury">
                            <span
                            ...
                            >
                              {line.total
                                ? `O${line.total} ${fmt(line.overPrice)}`
                                : "—"}
                            </span>
                          </td>
                        </tr>
              ...
              <div className="glass rounded-xl p-4 space-y-2">
                {keyFactors.slice(0, 6).map((factor: string, i: number) => (
                  <div key={i} className="flex items-start gap-2">
                    ...
                    <p className="text-xs text-mercury leading-relaxed">
                      {factor}
                    </p>
                  </div>
                ))}
          ...
          {!gameOdds && !analysis && (
            <div className="py-8 text-center">
              <p className="text-sm text-mercury/60">
                No detailed data available for this game yet.
              </p>
              <p className="text-[11px] text-mercury/40 mt-1">
                Check back after odds are posted.
              </p>
            </div>
          )}
        </div>
