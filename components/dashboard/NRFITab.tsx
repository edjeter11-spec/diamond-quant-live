...
export default function NRFITab() {
      ...
      <div className="space-y-3">
        <div className="glass rounded-xl p-5 border border-orange-500/15">
          ...
          <p className="text-xs text-mercury mb-2">
            First quarter scoring analysis for NBA games. Analyzes team pace,
            starters' scoring tendencies, and historical Q1 performance to
            project first quarter totals.
          </p>
          <div className="rounded-lg bg-gunmetal/30 p-4 text-center">
            ...
            <p className="text-sm text-mercury">
              Q1 engine processing today's games...
            </p>
  ...
  const topNRFI = upcoming
    ...
    .slice(0, 3);
  const topYRFI = upcoming
    .filter((g) => g.recommendation.includes("YRFI"))
    .slice(0, 3);
    ...
    return (
      <div className="max-w-4xl mx-auto">
        <div className="glass rounded-xl p-8 text-center">
          ...
          <p className="text-sm text-mercury">
            Analyzing first inning matchups...
          </p>
          <p className="text-[10px] text-mercury/50 mt-1">
            Checking pitcher NRFI rates, park factors, and K rates
          </p>
        </div>
      </div>
    );
      ...
      <div className="max-w-4xl mx-auto space-y-4">
        <div className="glass rounded-xl p-4 border border-electric/15">
          ...
          <p className="text-xs text-mercury">
            NRFI (No Run First Inning) and YRFI (Yes Run) predictions based on
            pitcher ERA, WHIP, K/9, park factors, and historical first inning
            performance.
          </p>
        </div>
        <div className="glass rounded-xl p-8 text-center">
          ...
          <p className="text-sm text-mercury">
            No MLB games available right now
          </p>
          <p className="text-[10px] text-mercury/50 mt-1">
            Check back closer to first pitch — NRFI picks generate when probable
            pitchers are posted
          </p>
        </div>
      </div>
      ...
      {topNRFI.length > 0 && (
        <div className="glass rounded-xl overflow-hidden border border-neon/15">
          <div className="px-4 py-3 bg-neon/5 border-b border-neon/15 flex items-center gap-2">
            ...
            <h3 className="text-xs font-bold text-neon uppercase tracking-wider">
              Top NRFI Plays
            </h3>
            <span className="text-[9px] text-mercury/50 ml-auto">
              Highest probability clean 1st innings
            </span>
          </div>
        ...
        <div className="divide-y divide-slate/10">
          {upcoming.length === 0 ? (
            <div className="p-6 text-center">
              <p className="text-sm text-mercury">
                No upcoming games with pitcher data
              </p>
            </div>
          ) : (
            upcoming.map((game) => (
      ...
      {live.length > 0 && (
        <div className="glass rounded-xl overflow-hidden">
          <div className="px-4 py-3 border-b border-slate/30 flex items-center gap-2">
            ...
            <h3 className="text-xs font-bold text-amber uppercase tracking-wider">
              In Progress
            </h3>
          </div>
          <div className="divide-y divide-slate/10">
  ...
  return (
    <div>
      ...
      >
        {/* Heatmap indicator */}
        <div
          className={`w-2 h-8 rounded-full ${heatColor} flex-shrink-0`}
          title={`Danger: ${game.dangerLevel}%`}
        />

        {/* Grade */}
        <div
          className={`w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold flex-shrink-0 ${grade.bg} ${grade.text} ${grade.border} border`}
        >
          {game.nrfiGrade}
        </div>
        ...
        <div className="text-right flex-shrink-0">
          <p
            className={`text-sm font-bold font-mono ${game.nrfiProb >= 60 ? "text-neon" : game.nrfiProb >= 50 ? "text-electric" : "text-danger"}`}
          >
            {game.nrfiProb}%
          </p>
        ...
        <div className="px-3 sm:px-4 pb-4 animate-slide-up space-y-3">
          ...
          <div className="flex items-center gap-2 px-1">
            ...
            <div className="flex-1 h-3 bg-gunmetal rounded-full overflow-hidden flex">
              <div
                className="h-full bg-neon/50 rounded-l-full"
                style={{ width: `${game.nrfiProb}%` }}
              />
              <div
                className="h-full bg-danger/50 rounded-r-full"
                style={{ width: `${game.yrfiProb}%` }}
              />
            </div>
  ...
  return (
    <div className="rounded-lg bg-gunmetal/30 p-2.5">
      ...
      <div className="grid grid-cols-2 gap-1 text-[9px]">
        ...
        <span className="text-mercury">
          K/9: <span className="text-silver font-mono">{pitcher.k9}</span>
        </span>
        <span className="text-mercury">
          WHIP:{" "}
          <span className="text-silver font-mono">
        ...
        <span className="text-mercury">
          NRFI:{" "}
          <span
            className={`font-mono font-bold ${pitcher.nrfiRate > 70 ? "text-neon" : pitcher.nrfiRate > 55 ? "text-electric" : "text-danger"}`}
          >
            {pitcher.nrfiRate}%
          </span>
        </span>
      </div>
      <p className="text-[8px] text-mercury/50 mt-1">
        {pitcher.last5FirstInning}
      </p>
    </div>
  );
}
