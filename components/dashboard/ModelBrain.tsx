...
export default function ModelBrain() {
  ...
  const markets = Object.values(state.marketAccuracy).filter(
    (m) => m.totalBets > 0,
  );
      ...
      <div className="px-3 sm:px-4 py-3 space-y-3">
        ...
        <div className="flex items-center gap-3 text-[11px]">
          ...
          <div className="flex items-center gap-1 text-mercury">
            ...
            <span>
              Optimized{" "}
              {timeSinceOptimized < 60
                ? `${timeSinceOptimized}m ago`
                : `${Math.floor(timeSinceOptimized / 60)}h ago`}
            </span>
          </div>
          <div className="flex items-center gap-1 text-mercury">
        ...
        {markets.length > 0 && (
          <div className="space-y-1.5">
            <p className="text-[9px] text-mercury uppercase tracking-wider font-semibold">
              Accuracy by Market (auto-adjusting thresholds)
            </p>
            {markets.map((m) => {
              const winRate =
                m.totalBets > 0 ? (m.wins / m.totalBets) * 100 : 0;
              const isGood = winRate > 52;
              return (
                <div key={m.market} className="flex items-center gap-2">
                  <span className="text-[10px] text-mercury w-20 truncate capitalize">
                    {m.market.replace("_", " ")}
                  </span>
                  ...
                  <span
                    className={`text-[10px] font-mono w-10 text-right ${isGood ? "text-neon" : "text-amber"}`}
                  >
                    {winRate.toFixed(0)}%
                  </span>
        ...
        <div>
          <p className="text-[9px] text-mercury uppercase tracking-wider font-semibold mb-1.5">
          ...
          <div className="flex gap-1">
            {Object.entries(state.weights).map(([key, val]) => (
              <div key={key} className="flex-1 text-center">
                ...
                <p className="text-[7px] text-mercury/50 mt-0.5 truncate">
                  {key.slice(0, 4)}
                </p>
              </div>
            ))}
