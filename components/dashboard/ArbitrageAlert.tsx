    ...
    <div className="glass rounded-xl overflow-hidden border border-gold/20">
      ...
      >
        ...
        <span className="text-[10px] text-gold/70">
          {arbitrage.length} {arbitrage.length === 1 ? "opp" : "opps"}
        </span>

        {/* Preview of best arb inline */}
        <div className="hidden sm:flex items-center gap-2 ml-auto">
          <span className="text-[11px] text-mercury truncate max-w-[200px]">
            {arbitrage[0].game}
          </span>
          <span className="text-xs font-bold font-mono text-gold">
            +{arbitrage[0].profit.toFixed(2)}%
          </span>
        </div>

        <span className="text-[10px] text-mercury/50 ml-auto sm:ml-2">
      ...
      {expanded && (
        <div className="p-2.5 sm:p-3 space-y-2 animate-slide-up border-t border-gold/10">
          {arbitrage.map((arb, i) => (
            <div
              key={i}
              className="flex items-center gap-2 sm:gap-3 px-2.5 py-2 rounded-lg bg-bunker/60 border border-gold/10"
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-mercury/50">
                  {arb.side1.pick} @ {arb.side1.bookmaker} (
                  {formatOdds(arb.side1.odds)}) vs {arb.side2.pick} @{" "}
                  {arb.side2.bookmaker} ({formatOdds(arb.side2.odds)})
                </p>
              </div>
              <div className="text-right flex-shrink-0">
                ...
                <p className="text-[9px] text-mercury/50">
                  ${arb.stake1.toFixed(0)} / ${arb.stake2.toFixed(0)}
                </p>
              </div>
            </div>
          ))}
