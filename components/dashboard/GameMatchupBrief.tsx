      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-slate-900/60 rounded-lg p-3 space-y-2">
          ...
          <div className="flex items-center gap-1.5 text-xs text-slate-300">
            ...
            <span
              className={
                data.rest.away.isB2B || data.rest.away.is3In4
                  ? "text-red-400"
                  : ""
              }
            >
          ...
          <div className="text-[11px] text-slate-400">
            Net {data.ratings.away.netRating >= 0 ? "+" : ""}
            {data.ratings.away.netRating.toFixed(1)} · Pace{" "}
            {data.ratings.away.pace.toFixed(1)}
          </div>
          {awayOuts.length > 0 && (
            <div className="flex items-start gap-1 text-[11px] text-red-300">
              ...
              <span>
                {awayOuts
                  .slice(0, 3)
                  .map((p) => `${p.name.split(" ").pop()} (${p.status[0]})`)
                  .join(", ")}
              </span>
            </div>
          )}
        ...
        <div className="bg-slate-900/60 rounded-lg p-3 space-y-2">
          ...
          <div className="flex items-center gap-1.5 text-xs text-slate-300">
            ...
            <span
              className={
                data.rest.home.isB2B || data.rest.home.is3In4
                  ? "text-red-400"
                  : ""
              }
            >
          ...
          <div className="text-[11px] text-slate-400">
            Net {data.ratings.home.netRating >= 0 ? "+" : ""}
            {data.ratings.home.netRating.toFixed(1)} · Pace{" "}
            {data.ratings.home.pace.toFixed(1)}
          </div>
          {homeOuts.length > 0 && (
            <div className="flex items-start gap-1 text-[11px] text-red-300">
              <AlertCircle className="w-3 h-3 mt-0.5 flex-shrink-0" />
