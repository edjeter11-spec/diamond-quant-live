...
export default function PropDetail({
    ...
    try {
      ...
      const prompt = {
        ...
        reasoning: [
          ...
          hitRate != null
            ? `Hits line in ${hitRate.toFixed(0)}% of recent games`
            : null,
        ...
        history:
          data?.last10Games
            ?.slice(0, 3)
            .map((g: any) => `${g.opponent}: ${getStatFromLog(g, market)}`) ??
          [],
        aiTip: `Give a 2-sentence assessment on whether to take this ${side} bet.`,
        gameId: `prop-${playerName}-${market}`
          .toLowerCase()
          .replace(/\s+/g, "-"),
      };
      ...
      setAiSummary(
        result.summary ??
          buildLocalAnalysis(data, sport, market, line, side, playerName),
      );
    } catch {
      setAiSummary(
        buildLocalAnalysis(data, sport, market, line, side, playerName),
  ...
  if (!data || data.error) {
    return (
      <div className="p-4 text-center">
        <p className="text-xs text-mercury/60">
          Stats unavailable for {playerName}.
        </p>
      </div>
    );
  }
  ...
  const last5Avg =
    last5.length > 0
      ? last5.reduce(
          (s: number, g: any) => s + (getStatFromLog(g, market) ?? 0),
          0,
        ) / last5.length
      : 0;
    ...
    <div className="p-3 space-y-3 animate-slide-up">
      ...
      <div className="grid grid-cols-4 gap-1.5">
        ...
        <StatTile
          ...
          accent={seasonAvg > line === (side === "over") ? "good" : "neutral"}
        />
        <StatTile
          label={gameLog.length > 0 ? "Last 5" : "Proj"}
          value={(gameLog.length > 0
            ? last5Avg
            : (brainProj?.projectedValue ?? seasonAvg)
          ).toFixed(1)}
          accent={
            (gameLog.length > 0 ? last5Avg : (brainProj?.projectedValue ?? 0)) >
              line ===
            (side === "over")
              ? "good"
              : "neutral"
      ...
      {isNBA && (nbaPlayer.fgPct || nbaPlayer.threePct || nbaPlayer.ftPct) && (
        <div className="grid grid-cols-3 gap-1.5">
          <StatTile
            label="FG%"
            value={`${((nbaPlayer.fgPct ?? 0) * 100).toFixed(1)}%`}
          />
          <StatTile
            label="3PT%"
            value={`${((nbaPlayer.threePct ?? 0) * 100).toFixed(1)}%`}
          />
          <StatTile
            label="FT%"
            value={`${((nbaPlayer.ftPct ?? 0) * 100).toFixed(1)}%`}
          />
        </div>
      )}

      {/* Brain projection card — renders for any sport once the brain weighs in */}
      {brainProj && (
        <div
          className={`rounded-lg p-3 border ${brainProj.side === side ? "border-neon/25 bg-neon/5" : "border-amber/25 bg-amber/5"}`}
        >
          <div className="flex items-center gap-1.5 mb-2">
            <Brain
              className={`w-3.5 h-3.5 ${brainProj.side === side ? "text-neon" : "text-amber"}`}
            />
            <span
              className={`text-[11px] font-bold uppercase tracking-wider ${brainProj.side === side ? "text-neon" : "text-amber"}`}
            >
              {brainProj.side === side
                ? `Brain agrees — ${side.toUpperCase()}`
                : `Brain leans ${brainProj.side.toUpperCase()}`}
            </span>
            {(() => {
              ...
              return (
                <span
                  className={`ml-auto text-[10px] px-1.5 py-0.5 rounded border ${tier.text} ${tier.bg} ${tier.border}`}
                >
                  {conf}% conf
                </span>
              );
          ...
          <div className="grid grid-cols-3 gap-1.5 mb-2">
            <StatTile
              label="Projected"
              value={(brainProj.projectedValue ?? 0).toFixed(1)}
              accent="good"
            />
            <StatTile
              label="Probability"
              value={`${Math.round((brainProj.probability ?? 0) * 100)}%`}
            />
            <StatTile
              label="Edge vs Line"
              value={`${(brainProj.projectedValue ?? 0) - line > 0 ? "+" : ""}${((brainProj.projectedValue ?? 0) - line).toFixed(1)}`}
              accent={
                (brainProj.projectedValue ?? 0) - line > 0 === (side === "over")
                  ? "good"
                  : "neutral"
              }
            />
          </div>
          {Array.isArray(brainProj.reasoning) &&
            brainProj.reasoning.length > 0 && (
              <ul className="space-y-1 mt-2 pt-2 border-t border-slate/20">
                {brainProj.reasoning.slice(0, 4).map((r: string, i: number) => (
                  <li
                    key={i}
                    className="text-[10px] text-mercury/80 flex items-start gap-1"
                  >
        ...
        <div className="rounded-lg bg-gunmetal/30 border border-slate/15 p-2.5">
            ...
            {gameLog
              ...
              .map((g: any, i: number) => {
                ...
                const max = Math.max(
                  ...gameLog
                    .slice(0, 10)
                    .map((x: any) => getStatFromLog(x, market) ?? 0),
                  line,
                  1,
                );
          ...
          <div className="flex items-center gap-2 mt-2 text-[9px] text-mercury/50">
            <span className="flex items-center gap-1">
              <span className="w-2 h-2 rounded-sm bg-neon/70 inline-block" />{" "}
              hit line
            </span>
            <span className="flex items-center gap-1">
              <span className="w-2 h-2 rounded-sm bg-mercury/30 inline-block" />{" "}
              missed
            </span>
          </div>
        </div>
      ...
      {recommendation && recommendation.reasons?.length > 0 && (
        ...
        >
          <div className="flex items-center gap-1.5 mb-1.5">
            <Brain
              className={`w-3.5 h-3.5 ${recommendation.side === side ? "text-neon" : "text-amber"}`}
            />
            <p
              className={`text-[11px] font-bold uppercase tracking-wider ${recommendation.side === side ? "text-neon" : "text-amber"}`}
            >
              {recommendation.side === side
...
function getSeasonAvg(data: any, sport: "mlb" | "nba", market: string): number {
  if (sport === "nba") {
    ...
    if (market === "batter_home_runs")
      return p.homeRuns && p.gamesPlayed ? p.homeRuns / p.gamesPlayed : 0;
...
function buildLocalAnalysis(
  ...
  const label =
    sport === "nba"
      ...
      : market === "pitcher_strikeouts"
        ? "strikeouts"
        : market.replace("batter_", "").replace(/_/g, " ");
  const p = data?.player ?? {};
  const seasonAvg = getSeasonAvg(data, sport, market);
  ...
  const parts: string[] = [];
  parts.push(
    `${playerName} averages ${seasonAvg.toFixed(1)} ${label} this season, ${abs} ${direction} the ${line} line.`,
  );
  if (projected != null) {
    const brainAgree = brain.side === side;
    parts.push(
      `The brain projects ${projected.toFixed(1)} (${conf ?? "—"}% confidence) — ${brainAgree ? "backing" : "fading"} the ${side}.`,
    );
