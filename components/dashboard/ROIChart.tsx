...
export default function ROIChart() {
  ...
  const chartData = useMemo(() => {
    ...
    for (const bet of settled) {
      const date =
        bet.timestamp?.split("T")[0] ?? new Date().toISOString().split("T")[0];
    ...
    return (
      <div className="glass rounded-xl p-4 text-center">
        ...
        <p className="text-xs text-mercury/50">
          Need 2+ days of settled bets to show ROI chart
        </p>
      </div>
    );
  }
  ...
  const pathPoints = chartData.map(
    (d, i) => `${xScale(i)},${yScale(d.cumulative)}`,
  );
  ...
  const strokeColor = isPositive ? "#2ee6a6" : "#ff5c7a";
  const fillColor = isPositive ? "#2ee6a615" : "#ff5c7a15";
  ...
  return (
    <div className="glass rounded-xl overflow-hidden">
      <div className="px-4 py-2.5 border-b border-slate/20 flex items-center gap-2">
        ...
        <span
          className={`ml-auto text-sm font-bold font-mono ${isPositive ? "text-neon" : "text-danger"}`}
        >
      ...
      <div className="p-3">
        ...
        >
          ...
          <circle
            cx={xScale(0)}
            cy={yScale(chartData[0].cumulative)}
            r="3"
            fill={strokeColor}
          />
          <circle
        ...
        <div className="flex items-center justify-between mt-2 text-[9px] text-mercury/50">
          ...
          <span>
            Best: +${Math.max(...chartData.map((d) => d.pl)).toFixed(0)} |
            Worst: ${Math.min(...chartData.map((d) => d.pl)).toFixed(0)}
          </span>
        </div>
      </div>
    </div>
